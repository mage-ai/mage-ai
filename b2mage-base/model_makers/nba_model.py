...
def train(df: pd.DataFrame, *args, **kwargs):
    """
    Train an XGBoost ranker model for recommendation tasks.
    ...
    categorical_columns: list  — columns to ordinal-encode; auto-detected
                                 from object/category dtypes if not given   (default: auto-detect)
    ...
    objective          : str   — XGBoost ranking objective                  (default: 'rank:pairwise')
    ...
    objective = kwargs.get('objective', 'rank:pairwise')
    ...
    # Ordinal-encode categoricals (XGBoost requires numeric input)
    ...
    # XGBRanker requires rows sorted by group and group sizes as a 1-D array
    ...
    if IS_SAVE:
        ...
        filename = f"recommendation_xgboost_{datetime.now().strftime('%Y%m%d')}.json"
