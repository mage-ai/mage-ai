...
def generate_models_sql(catalog_data: dict, schema_name: str) -> list[tuple[str, str]]:
    ...
    for stream in catalog_data["catalog"]["streams"]:
        ...
        for column in stream["schema"]["properties"].keys():
            sql += f"  {column},\n"

        # Remove the trailing comma and newline character
        sql = sql[:-2] + "\n"

        sql += "FROM {{ source('" + schema_name + "', '" + stream_name + "') }} \n"
...
def update_dbt_sources(catalog_data: dict, sources_file: str, schema_name: str):
    ...
    for stream in catalog_data["catalog"]["streams"]:
        table_name = stream["destination_table"]
        table_data = {
            "name": table_name,
            "description": f"Table containing {table_name}",
            "loaded_at_field": "_mage_created_at",
        }
        yaml_data["sources"][schema_index]["tables"].append(table_data)
