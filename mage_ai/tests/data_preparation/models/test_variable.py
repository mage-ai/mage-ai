...
class VariableTest(DBTestCase):
    ...
    def test_write_and_read_geodataframe(self):
        ...
        df2 = gpd.GeoDataFrame(
            [
                [1, 'test', 3.123, np.NaN, MultiPoint([Point(0, 1), Point(0, 1)])],
                [2, 'test2', 4.321, np.NaN, MultiPoint([Point(0, 1), Point(0, 1)])],
            ],
            columns=['col1', 'col2', 'col3', 'col4', 'geometry']
        )
        df2['col4'] = df2['col4'].astype('Int64')
        variable1.write_data(df1)
        variable2.write_data(df2)
        variable_dir_path = os.path.join(pipeline.dir_path, '.variables')
        self.assertTrue(os.path.exists(
            os.path.join(variable_dir_path, 'block1', 'var1', 'geodata.parquet'),
        ))
        self.assertTrue(os.path.exists(
            os.path.join(variable_dir_path, 'block1', 'var1', 'sample_data.parquet'),
        ))
        self.assertTrue(os.path.exists(
            os.path.join(variable_dir_path, 'block2', 'var2', 'geodata.parquet'),
        ))
        self.assertTrue(os.path.exists(
            os.path.join(variable_dir_path, 'block2', 'var2', 'sample_data.parquet'),
        ))
    ...
    def test_write_and_read_json(self):
        pipeline = self.__create_pipeline('test pipeline 5')
        variable = Variable(
            'var1',
            pipeline.dir_path,
            'block1',
