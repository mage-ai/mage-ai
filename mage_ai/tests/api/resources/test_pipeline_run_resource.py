...
class PipelineRunPresenterQueuePositionTest(BaseApiTestCase):
    ...
    def setUp(self):
        ...
        try:
            ...
            self.pipeline = Pipeline.get(
                'test_queue_pipeline',
                repo_path=self.repo_path,
            )

        self.schedule = PipelineSchedule.create(
            name=self.faker.unique.name(),
    ...
    def tearDown(self):
        PipelineRun.query.filter(
            PipelineRun.pipeline_schedule_id == self.schedule.id,
        ).delete()
        self.schedule.delete()
        self.pipeline.delete()
        super().tearDown()

    def _create_run(self, status=PipelineRun.PipelineRunStatus.INITIAL):
        return PipelineRun.create(
            pipeline_schedule_id=self.schedule.id,
            pipeline_uuid=self.pipeline.uuid,
            status=status,
        )
    ...
    def test_queue_position_map_position_decrements(self):
        ...
        self.assertEqual(result[run3.id], 2)
