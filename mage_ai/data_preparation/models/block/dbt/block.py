...
from mage_ai.data_preparation.models.block.dbt.handler import DBTHandler
from mage_ai.data_preparation.models.block.dbt.profile import DBTProfileHandler
from mage_ai.data_preparation.models.block.dbt.upstream import DBTUpstreamHandler
...
class DBTBlock(Block):
    ...
    async def metadata_async(self) -> Dict:
        ...
        if self.configuration.get('file_path'):
            attributes_dict = DBTProfileHandler.parse_attributes(
                self,
            ...
            targets = []
            profiles = await DBTProfileHandler.load_profiles_async(
                profile_name,
                profiles_full_path,
                variables=self.pipeline.variables if self.pipeline else None,
            )
            ...
            for project_name_init in project_names:
                ...
                info = DBTProfileHandler.get_dbt_project_name_from_settings(project_name_init)
    ...
    def run_tests(
        ...
        **kwargs,
        ...
        if self.configuration.get('file_path') is not None:
            attributes_dict = DBTProfileHandler.parse_attributes(self, variables=global_vars)
    ...
    def update_upstream_blocks(
        ...
        **kwargs,
        ...
        if BlockLanguage.SQL == self.language:
            DBTUpstreamHandler.update_model_settings(
                self,
                upstream_blocks,
    ...
    def _execute_block(
        ...
        **kwargs,
        ...
        dbt_command, args, command_line_dict = DBTHandler.build_command_line_arguments(
            self,
            variables,
        ...
        try:
            ...
            if is_sql:
                DBTUpstreamHandler.create_upstream_tables(
                    self,
            ...
            if is_sql and from_notebook:
                ...
                if not snapshot:
                    df = DBTHandler.query_from_compiled_sql(
                        self,
                        dbt_profile_target,
            ...
            if not from_notebook:
                target_path = None
                if self.configuration.get('file_path') is not None:
                    attributes_dict = DBTProfileHandler.parse_attributes(self, variables=variables)
                    target_path = attributes_dict['target_path']

                if snapshot and BlockLanguage.SQL == self.language:
                    query_string = DBTHandler.compiled_query_string(self, variables=variables)
                    if query_string:
                ...
                if is_sql and dbt_command in ['build', 'run']:
                    ...
                    try:
                        df = DBTHandler.fetch_model_data(
                            self,
                            dbt_profile_target,
                            limit=limit,
                            variables=variables,
    ...
    # methods references from outside, which should also be interfaces

    """
    Used by Block after create trigger to update Pipeline and add upstream models as Blocks

    Used from the UI with Menu in single dbt models to add upstream models manually after a change
    of ref

    Used to get all dbt upstream models for the BlockPresenter, which not actually needs
    to run the function itself, but just needs to get Blocks from the pipeline to pass to Lineage
    tab
    - Lineage Tab does not give any additional information, which the graph does not have yet
    - Lineage Tab might show the difference to the between the Mage Graph and the most up to date
      dbt graph
    """
    @classmethod
    def add_blocks_upstream_from_refs(
        ...
        return DBTUpstreamHandler.add_blocks_upstream_from_refs(
            block,
            add_current_block,
            downstream_blocks,
            read_only,
    ...
    """
    Gets the compiled query to show in the SQL Tab under a dbt model
    """
    @classmethod
    def compiled_query_string(
        ...
        return DBTHandler.compiled_query_string(block, error_if_not_found, variables)

    """
    Used to handle sources.yml file if block gets added/deleted from pipeline
    """
    @classmethod
    ...
    """
    Used to create tables for upstream blocks, which are not dbt models and should be created as
    dbt source

    Executes currently the create_upstream_block_tables method of the internal database connector,
    as its used from both dbt and raw sql files
    """
    @classmethod
    def parse_attributes(cls, block, variables: Dict = None) -> Dict:
        return DBTProfileHandler.parse_attributes(block, variables)

    @classmethod
    def source_table_name_for_block(cls, block) -> str:
        return DBTUpstreamHandler.source_table_name_for_block(block)
