...
import { LOCAL_STORAGE_KEY_PIPELINE_EXECUTION_HIDDEN, set } from '@storage/localStorage';
...
function PipelineExecution({
  ...
  }, [pipelineExecutionHidden, setPipelineExecutionHidden, treeHidden]);
    ...
    <>
        ...
        <FlexContainer alignItems='center' justifyContent='space-between'>
          <Flex>
            <Button
              ...
              success
            >
              <Text bold inverted noWrapping primary={false}>
                Execute pipeline
              </Text>
            </Button>
            <Spacing ml={1} />
            ...
            <Button onClick={checkIfPipelineRunning} secondary>
          ...
          <Flex alignItems='center'>
            {setTreeHidden && (
              <>
                <Text>Tree</Text>
                <Spacing mr={1} />
                <span data-testid='tree-toggle'>
                  <ToggleSwitch
                    checked={!treeHidden}
                    disabled={pipelineExecutionHidden && !treeHidden}
                    onCheck={() => {
            ...
            <Text>Output</Text>
            <Spacing mr={1} />
            <span data-testid='output-toggle'>
              <ToggleSwitch
                checked={!pipelineExecutionHidden}
                disabled={treeHidden && !pipelineExecutionHidden}
      ...
      {!pipelineExecutionHidden && (
        <>
          ...
          >
            <CodeBlockStyle executedAndIdle hasError={false} selected>
              {truncatedPipelineMessages.map(
                ({ data: dataInit, type: dataType }: KernelOutputType, idx: number) => {
                  ...
                  return dataArray.map((data: string, idxInner: number) => {
                    ...
                    return <div key={`code-output-${idx}-${idxInner}`}>{displayElement}</div>;
                  });
                },
              )}
            </CodeBlockStyle>
