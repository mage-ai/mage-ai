...
import { LOCAL_STORAGE_KEY_DATA_OUTPUT_BLOCK_UUIDS, get, remove, set } from '@storage/localStorage';
import {
  addUnderscores,
  isJsonString,
  randomNameGenerator,
...
export function prepareOutput(output) {
  ...
  if (typeof output === 'object') {
    const { sample_data: sampleData, shape: shape, text_data: textDataJsonString } = output || {};
    type = output?.type;
...
export function prepareOutputsForDisplay(outputs: OutputType[]) {
  ...
  if (multiOutput) {
    ...
    DataTypeEnum.TABLE === outputType &&
    outputsFinal?.length >= 2 &&
    !!outputsFinal?.[0]?.data?.text_data
  ) {
...
export function initializeContentAndMessages(blocks: BlockType[]) {
  ...
  blocks?.forEach(({ content, outputs, type, uuid }: BlockType) => {
    const groupedOrMultiOutputs: boolean =
      outputs?.length >= 1 &&
...
export function updateCollapsedBlockStates(
  blocks: BlockType[],
  pipelineUUID: string,
  newPipelineUUID: string,
) {
  blocks.forEach(b => {
    set(
...
export function addDataOutputBlockUUID(pipelineUUID: string, blockUUID: string) {
  const blockUUIDs = getDataOutputBlockUUIDs(pipelineUUID);
  if (!blockUUIDs.includes(blockUUID)) {
    set(getOutputBlockUUIDStorageKey(pipelineUUID), [...blockUUIDs, blockUUID]);
  }
}

export function removeDataOutputBlockUUID(pipelineUUID: string, blockUUID: string) {
  const blockUUIDs = getDataOutputBlockUUIDs(pipelineUUID);
...
export function convertBlockUUIDstoBlockTypes(uuids: string[], blocks: BlockType[]): BlockType[] {
  const blockUUIDMapping = indexBy(blocks, block => block.uuid);
  return uuids.map(uuid => blockUUIDMapping[uuid]).filter(block => !!block);
}
...
export function displayPipelineLastSaved(
  ...
  if (pipelineContentTouched) {
    ...
    if (opts?.displayRelative && now - pipelineLastSaved < 60 * 60) {
      const ago = moment
        .unix(now - (now - pipelineLastSaved))
        .utc()
        .fromNow();
    ...
    const lastSavedDate = momentInLocalTimezone(
      moment(opts?.showLastUpdatedTimestamp),
      displayLocalTimezone,
    );
    saveStatus = `Last saved ${lastSavedDate}`;
  } else {
...
export function buildBlockRefKey({ type, uuid }: { type?: string; uuid?: string }): string {
...
export function buildBlockFromFilePath({
  ...
  const projectPath = `${repoPathRelativeRoot}${osPath.sep}${BlockFolderNameEnum.DBT}${osPath.sep}`;
