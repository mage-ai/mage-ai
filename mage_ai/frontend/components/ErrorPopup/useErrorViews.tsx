...
export default function useErrorViews({
  ...
  const displayMessageMemo = useMemo(() => displayMessage && (
    <Text default>
      {displayMessage}
    </Text>
  ), [displayMessage]);

  const exceptionMemo = useMemo(() => exception && (
    <Text
      default
      disableWordBreak
      monospace
  ...
  const tracebackMemo = useMemo(() => messages?.length > 0 && (
    <>
      <Text bold large>
        Traceback (<Link
  ...
  const stackTraceMemo = useMemo(() => (errors && errors?.[0] !== '') && (
    <>
      <Text bold large>
        Stack trace (<Link
  ...
  const linksMemo = useMemo(() => links?.map(({ closeAfterClick, href, label, onClick }, idx) => (
