...
export default function useToolbars({
  resource, stale, updateLocalContent, updateServerContent,
}: ToolbarsType) {
  const inputRef = useRef<HTMLInputElement>(null);
  const { main, original } = resource;

  const saveCurrentContent = useCallback((event: MouseEvent) => {
    updateServerContent(event, main, {
      path: inputRef?.current?.value || main?.path,
    });
  }, [main, updateServerContent])

  const overrideServerContentFromLocal =
    useCallback((event: MouseEvent) => updateServerContent(event, main, main), [main, updateServerContent]);

  const overrideLocalContentFromServer =
    useCallback(() => updateLocalContent(original), [original, updateLocalContent]);
