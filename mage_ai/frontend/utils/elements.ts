...
export function removeClassNames(className: string, classNames: (string[] | ((className: string) => boolean))): string {
  const arr = (className || '')?.split(' ')?.filter(cn => typeof classNames === 'function'
    ? !classNames(cn)
    : !classNames?.includes(cn),
  );
