  ...
  useEffect(() => {
    Object.entries(elementMapping || {})?.forEach(([uuid, element]: [ApplicationExpansionUUIDEnum, ElementType]) => {
      const Resize = (e) => {
        ...
        if (!refOrientationMapping?.current?.[uuid] || typeof refOrientationMapping?.current?.[uuid] === 'undefined') {
          ...
          if (isLeft) {
            ...
            if (!refRecentValuesMapping?.current?.[uuid]?.left ||
              (
                ...
                (left2 >= 24 || (left2 > refRecentValuesMapping?.current?.[uuid]?.left && clientX >= offsetLeft))
                &&
                // Width more than 400 or... moving to the left while being 400 pixels away from the right side
                (width2 >= 400 || (left2 < refRecentValuesMapping?.current?.[uuid]?.left && clientX <= ((offsetLeft + width2) - 400)))
              )
            ) {
              ...
              // If there is no previous value
              if (!refRecentValuesMapping?.current?.[uuid]?.width ||
                (
            ...
            if (!refRecentValuesMapping?.current?.[uuid]?.width ||
              (
                ...
                width2 > refRecentValuesMapping?.current?.[uuid]?.width && offsetLeft + width2 <= (window.innerWidth - (24 * 2))
              )
          ...
          if (top2) {
            ...
            if (!refRecentValuesMapping?.current?.[uuid]?.top ||
              (
                ...
                (top2 >= 24 || (top2 > refRecentValuesMapping?.current?.[uuid]?.top && clientY >= offsetTop))
              )
              &&
              // Height more than 300 or.. moving up while being 300 pixels away from the bottom side
              (height2 >= 300 || (top2 < refRecentValuesMapping?.current?.[uuid]?.top && clientY <= ((offsetTop + height2) - 300)))
          ) {
              ...
              // If there is no previous value
              if (!refRecentValuesMapping?.current?.[uuid]?.height ||
                (
            ...
            if (!refRecentValuesMapping?.current?.[uuid]?.height ||
              (
                ...
                height2 > refRecentValuesMapping?.current?.[uuid]?.height && offsetTop + height2 <= (window.innerHeight - (24 * 2))
              )
