...
def build_load(associated_resource_class, **kwargs):
    associated_id_column_name = '{}_id'.format(
        kwargs.get('attribute', associated_resource_class.resource_name_singular()),
    )
    ...
    def _load(resource):
        ids = list(
            filter(
                lambda x: x,
                [getattr(r, associated_id_column_name) for r in resource.result_set()],
            )
        )
        ...
        return getattr(
            importlib.import_module('api.resources.{}'.format(associated_resource_class.__name__)),
            associated_resource_class.__name__,
        ).build_result_set(query_set, resource.current_user)
...
def build_load_select(current_resource, associated_resource_class, **kwargs):
    ...
    def _load(resource):
        ...
        query_set = associated_resource_class.model_class.objects.filter(**query)
        return getattr(
            importlib.import_module('api.resources.{}'.format(associated_resource_class.__name__)),
            associated_resource_class.__name__,
        ).build_result_set(query_set, resource.current_user)
...
def load_user(resource):
    ...
    UserResource = getattr(  # noqa: B009
