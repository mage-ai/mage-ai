...
def is_owner(user) -> bool:
    return (user and user.owner) or (not REQUIRE_USER_AUTHENTICATION and not is_test())


def has_at_least_admin_role(user) -> bool:
    return (
        not user
        or (not REQUIRE_USER_AUTHENTICATION and not is_test())
        or is_owner(user)
        or (user.roles and user.roles & 1 != 0)
    )
...
def has_at_least_editor_role(user) -> bool:
    return (
        ...
        or (not REQUIRE_USER_AUTHENTICATION and not is_test())
        or is_owner(user)
        or has_at_least_admin_role(user)
        or (user.roles and user.roles & 2 != 0)
    )
...
def has_at_least_viewer_role(user) -> bool:
    return (
        ...
        or is_owner(user)
        or has_at_least_admin_role(user)
        or has_at_least_editor_role(user)
        or (user.roles and user.roles & 4 != 0)
    )
