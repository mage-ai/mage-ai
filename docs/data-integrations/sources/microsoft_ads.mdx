...
| `insights_lookback_window` | | `0` | Days to rewind from the saved report bookmark when syncing performance reports. Never goes earlier than `start_date`. |
| `ad_labels_max_ads` | | `10000` | Maximum current ads to scan per account when syncing `ad_labels`; increase only when large-account sync time and API quota are acceptable. |
| `ad_labels_max_hierarchy_requests` | | `1000` | Maximum campaign/ad group/ad hierarchy API requests per account when syncing `ad_labels`. |
...
| `ad_labels` | [Campaign Management label associations](https://learn.microsoft.com/en-us/advertising/guides/labels) | Full-table rows for current ad label associations. |

Performance report streams sync **incrementally** on `TimePeriod` (one bookmark per ad account). On the first run the full history from `start_date` is fetched; subsequent runs fetch only new or updated days. The `ad_labels` stream syncs as **full table** from the current label associations returned by the Campaign Management API.
