...
1. Run the following command in PostgreSQL to create a replication slot:
    ...
    <sub>The replication slot name `mage_slot` quoted is used as an example in the guide. The actual replication slot name should be unique for pipeline source. </sub>
