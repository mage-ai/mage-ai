...
This source syncs ad management objects, audience objects, and performance insight reports from the [TikTok Marketing API](https://ads.tiktok.com/marketing_api/). It covers advertisers, campaigns, ad groups, ads, audiences, and a set of insight breakdowns (by age/gender, country, platform, and province).
...
Insight streams (`ad_insights`, `ad_insights_by_*`) sync **incrementally** from `start_date`. Entity streams (`advertisers`, `campaigns`, `adgroups`, `ads`, `audiences`) sync **full table** by default.
...
| `insights_lookback_window` | | `0` | Days to rewind from the saved reporting bookmark for insight streams. Never goes earlier than `start_date`. |
...
| `audiences` | Full table | Custom audiences per advertiser, including `ad_type`. Requires TikTok Audience Management read permission. |
