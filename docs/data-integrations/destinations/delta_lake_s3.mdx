...
Mage Pro Data Preview, Data Explorer, AI Sidekick exploration, and Analytics
chart datasets can read a bounded window from a Delta Lake table on S3 when the
source is configured as `delta_lake_s3`.

Preview supports either a full table URI:

```yaml
provider: delta_lake_s3
uri: s3://analytics-lake/warehouse/orders
```
...
object_key_path: warehouse
table: orders
```

Mage reads AWS credentials from the selected `io_config.yaml` profile when one
is provided. Use a profile with read access to the target table path.
