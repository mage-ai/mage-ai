...
<div className="no-border-bottom" id="no-border-bottom">
  <div
    style={{ height: 32, position: 'relative' }}
  >
    <a
      target="_blank"
      className="group px-4 py-1.5 relative inline-flex items-center text-sm font-medium no-border-bottom"
      href={href ?? `https://mage.ai/pro?source=${source}`}
    >
      <span
