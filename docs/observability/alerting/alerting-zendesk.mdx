...
After you create your Zendesk API token, you can configure alerts in the Mage Pro
UI instead of editing `metadata.yaml` directly:
...
3. Choose the alert outcomes, enable Zendesk, enter the subdomain, email, and
   API token, and use **Send test notification** to create a test ticket before
   saving.

You can still reference secrets with values such as
`{{ env_var('MAGE_ZENDESK_API_TOKEN') }}`.
