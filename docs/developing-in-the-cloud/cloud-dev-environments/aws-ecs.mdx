...
The Mage application runs as multiple containers in an Amazon ECS instance. 
...
We use EFS to store a persistent volume where the `mage_data` for your project is stored. You can learn more about what is stored in `mage_data` [here](/design/core-abstractions#project-data). Without EFS, your project data would be lost when your ECS instance is terminated. EFS allows us to persist data between containers within ECS.
...
We use a Postgres RDS to hold the the backend database for your Mage instance. This database contains the metadata that powers Mage. Read more [here](/design/core-abstractions#project-data).
...
The Mage applicaiton, ALB, ECS, EFS, and RDS are all deployed within a VPC in our Terraform scripts.
