...
service account, you can run the following command:
...
* [Create your service account secrets](https://cloud.google.com/sql/docs/postgres/connect-kubernetes-engine#service-account-key-file)
  * Make sure to record the name of the service account credential file. 
...
* DB_SECRETS_NAME="db secrets name (required for PostgreSQL DB)"
* SERVICE_ACCOUNT_SECRETS_NAME="service account secrets name (required for PostgreSQL DB)"
* SERVICE_ACCOUNT_CREDENTIAL_FILE_PATH="credential file path (required for PostgreSQL DB)"
