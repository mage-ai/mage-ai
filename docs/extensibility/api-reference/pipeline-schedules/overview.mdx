...
<ResponseField name="event_matchers" type="array of objects">
  Deprecated. Legacy event matching logic returned for existing event triggers. New trigger
  creation supports `api` and `time` schedule types.
...
<ResponseField name="name" type="string" required>
  Name of trigger. Names are trimmed before validation, must include at least one letter or
  number, cannot exceed 255 characters, and must be unique within the pipeline.
</ResponseField>

<ResponseField name="next_pipeline_run_date" type="datetime">
...
<ResponseField name="schedule_type" type="string" required>
  Trigger type. Supported new trigger creation uses `api` or `time`; `event` may appear for
  deprecated legacy event triggers.
</ResponseField>
...
<ResponseField name="settings" type="object">
  ...
  <Expandable title="properties" defaultOpen="true">
    ...
    <ResponseField name="timeout" type="integer">
      Optional run timeout in seconds. When provided, the value is a positive integer. A `null`
      or omitted value means no timeout.
    </ResponseField>
  </Expandable>
</ResponseField>

<ResponseField name="sla" type="integer">
  Optional SLA threshold in seconds. If the pipeline run has been running for more than the SLA
  value, then the pipeline will fail. A positive integer enables SLA enforcement; `0` clears the
  SLA on update.
</ResponseField>
