...
<ParamField body="workspace.lifecycle_config" type="string">
  Optional. YAML configuration string for lifecycle hooks. Used with the `patch` action.
  Defines commands to run at different stages of the container lifecycle (e.g., postStart, preStop hooks).
</ParamField>
...
- `workspace.lifecycle_config` (string) - YAML configuration string for lifecycle hooks
