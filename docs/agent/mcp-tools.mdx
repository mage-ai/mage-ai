...
<ProOnly source="mage-agent-mcp-tools" />
...
<AccordionGroup>
  <Accordion title="current_page_context">
    ...
    This tool is only registered when Mage Pro AI Chat starts with captured page context. Standard IDE MCP sessions do not expose it unless `MAGE_AGENT_CURRENT_PAGE_CONTEXT_PATH` points to a valid context sidecar file.
    ...
    <Note>
      The context is scoped to the authenticated user and chat session, excludes the chat UI, and should only be used for page-relative requests such as "explain this page" or "what am I looking at?"
    </Note>
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="block_create">
    ...
    **Supported block types:**
    `ai`, `data_loader`, `transformer`, `data_exporter`, `sensor`, `callback`, `chart`, `conditional`, `custom`, `dbt`, `extension`, `global_data_product`, `markdown`, `scratchpad`

    AI blocks are available in Mage Pro only. For Sidekick-powered agent blocks, use `type="ai"`, `language="yaml"`, and YAML content with top-level `type: agent`.
