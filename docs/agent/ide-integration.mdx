...
<Note>
  Codex CLI commands that you run manually from the Mage terminal use the
  terminal process environment and Codex config directly, including `CODEX_HOME`,
  provider credential variables, and any configured `mcpServers`. Mage Pro AI
  Chat still uses Mage Pro API proxy authentication for Sidekick-managed Codex
  sessions.
</Note>
