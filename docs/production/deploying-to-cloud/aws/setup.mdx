...
By default, here are the **[resources](/d/deploying-to-cloud/aws/resources)** that will be
created.
...
Similar with deploying to ECS cluster, you need to set up the [environment variables](https://docs.mage.ai/production/deploying-to-cloud/aws/setup#1-environment-variables) first, and you can customize the Terraform settings in [./aws-eks/variables.tf](https://github.com/mage-ai/mage-ai-terraform-templates/blob/master/aws-eks/variables.tf) file.


Then, follow the steps below to set up EKS cluster and deploy Mage.
1. Run `terraform apply` to provision the EKS cluster
2. Configure `kubectl` with command `aws eks --region $(terraform output -raw region) update-kubeconfig --name $(terraform output -raw cluster_name)`
3. Update the `EFS_VOLUME_ID` in [kube/efs-pvc.yaml](https://github.com/mage-ai/mage-ai-terraform-templates/blob/master/aws-eks/kube/efs-pvc.yaml) to the efs id in terraform output
4. Run command `kubectl apply -k "github.com/kubernetes-sigs/aws-efs-csi-driver/deploy/kubernetes/overlays/stable/?ref=master"` to install efs csi driver
5. Run `kubectl apply -f kube/` to deploy app
6. Whitelist IP in EKS node's security group
7. Get the node IP via command `kubectl get nodes -o wide` and access Mage with `http://[ip]:31479`
