...
Use environment variables to set default resource requests, resource limits,
maximum allowed resource requests, maximum allowed resource limits, and service
account for all Kubernetes executor jobs. Default values are only applied when a
more specific project, pipeline, or block config does not set the same resource
group. Maximum values are Mage Pro-only guardrails: after environment defaults,
project config, pipeline config, and block config are merged, Mage fails the job
before creating the Kubernetes Job if the effective request or limit exceeds a
configured maximum.
...
export K8S_EXECUTOR_MAX_RESOURCE_REQUESTS_CPU=2000m
export K8S_EXECUTOR_MAX_RESOURCE_REQUESTS_MEMORY=4Gi
export K8S_EXECUTOR_MAX_RESOURCE_LIMITS_CPU=4000m
export K8S_EXECUTOR_MAX_RESOURCE_LIMITS_MEMORY=8Gi
...
values in the pipeline monitoring page. Open **Monitoring** for a pipeline, then
review **Pipeline resource usage** for compute metrics history and **Pipeline
Recommended K8s executor config** for the recommended Kubernetes executor
config. Use **Apply** to write the recommendation to the pipeline-level
`executor_config`, or copy the values into block-level `executor_config` or
project-level `k8s_executor_config` when you want a narrower or broader scope.
...
The Kubernetes Metrics API fallback samples active pods only. To use it, install
Metrics Server, confirm `v1beta1.metrics.k8s.io` is available, and grant the
Mage service account `get` and `list` access to `pods.metrics.k8s.io` in the K8s
executor job namespace. Because the Metrics API does not keep historical pod
samples after pods finish, short runs can still have sparse metrics; use
Prometheus when you need reliable completed-run history.
...
See [Monitoring with Prometheus](/observability/external/prometheus#kubernetes-resource-usage-in-mage-pro) for Prometheus query behavior, authentication details, and Kubernetes Metrics API fallback setup.
...
These environment variables set default resource limits and requests, maximum
allowed resource limits and requests, and the default service account for all
K8s executor jobs. Defaults are applied when project-level, pipeline-level, or
block-level configuration does not specify resource requests or limits.
Maximums are Mage Pro-only guardrails enforced after environment defaults,
project-level config, pipeline-level config, and block-level config are merged.
...
| `K8S_EXECUTOR_MAX_RESOURCE_REQUESTS_CPU`      | Maximum CPU request allowed for executor jobs | `2000m`    |
| `K8S_EXECUTOR_MAX_RESOURCE_REQUESTS_MEMORY`   | Maximum memory request allowed for executor jobs | `4Gi`   |
| `K8S_EXECUTOR_MAX_RESOURCE_LIMITS_CPU`        | Maximum CPU limit allowed for executor jobs | `4000m`     |
| `K8S_EXECUTOR_MAX_RESOURCE_LIMITS_MEMORY`     | Maximum memory limit allowed for executor jobs | `8Gi`    |
