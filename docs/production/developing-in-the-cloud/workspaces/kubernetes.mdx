...
The most common ingress controller is the nginx ingress controller . You
can find instructions on how to install the controller [here](https://kubernetes.github.io/ingress-nginx/deploy/).

You can set up a default ingress through the Mage Helm chart by setting the following fields in the
`values.yaml` file:
