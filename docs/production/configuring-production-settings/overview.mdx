...
<ProOnly source="production-settings" />

If Mage fails to start because an orchestration database migration failed, use `mage db diagnose` first. See [Troubleshoot database migration failures](/development/databases/default#troubleshoot-database-migration-failures) for migration history, table schema inspection, migration, rollback, and Alembic version stamping commands.
