...
If your Mage app is deployed on GCP, you can choose to launch separate GCP Cloud Run jobs to executor blocks by specifying
block executor_type to be `gcp` in pipeline's metadata.yaml file. Customizing compute resource
for GCP Cloud Run executor is coming soon.

Example pipeline block config:
