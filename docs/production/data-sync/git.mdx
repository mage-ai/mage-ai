...
[guide](/getting-started/setting-up-git#3-set-up-ssh-key)
to set up your SSH key.
1. Select SSH as the authentication type.
2. Fill out the form:
...
3. Fill out the remaining _optional, but recommended_ fields. Filling out these fields will allow Mage to
...
4. Fill out the _optional_ sync fields if you are using a [Git sync](/production/data-sync/git#git-sync)
...
7. Fill out the _optional_ sync fields if you are using a [Git sync](/production/data-sync/git#git-sync)
   (this is a separate Git feature):
    1. `Branch name`: This field is only used when running a Git sync. Git syncs can be run either from this
        Git settings page, or before every trigger run.
    2. `Sync before each trigger run`: If this field is checked, then Mage will sync with the Git repo before
        each trigger is run.
    2. `Sync on server start up`: If this field is checked, then Mage will sync with the Git repo when
        Mage is started.
...
* GIT_SYNC_ON_PIPELINE_RUN: corresponds to `Sync before each trigger run`, options: 0 or 1
* GIT_SYNC_ON_START: corresponds to `Sync on server start up`, options: 0 or 1
* GIT_SYNC_ON_EXECUTOR_START: If set to "1", Mage will attempt to run a git sync every time an executor is started, options: 0 or 1
