...
<ProOnly source="troubleshooting" />

If Mage fails to start because an Alembic or SQLAlchemy database migration failed, run `mage db diagnose` to inspect the current migration state and get the next recommended command.

See [Troubleshoot database migration failures](/development/databases/default#troubleshoot-database-migration-failures) for `mage db history`, `mage db schema`, `mage db migrate`, `mage db rollback`, and `mage db stamp` recovery steps.
