...
| [`clean-workspaces`](#clean-workspaces-clean-workspaces) | Clean up old pipeline run outputs and logs inside workspace pods | ❌ | ✅ |
...
Clean up old pipeline run outputs and logs inside Kubernetes workspace pods.

**Usage:**
```bash
mage clean-workspaces [OPTIONS]
```

**Parameters:**
- `--cluster-type` (optional): Cluster type for workspace cleanup. Currently only `k8s` is supported
- `--workspace-name` (optional): Workspace name to clean. If not specified, cleans all workspaces
- `--pipeline-uuid` (optional): UUID of the pipeline to clean
- `--retention-period` (optional): Default retention period, e.g., `7d`, `2w` (days or weeks)
- `--skip-unavailable / --no-skip-unavailable` (optional): Skip workspaces without a running pod instead of failing the cleanup job

**Example:**
```bash
mage clean-workspaces --retention-period 30d
```
