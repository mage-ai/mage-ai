...
It can be customized by setting the `SERVER_LOGGING_FORMAT` environment variable.
Example:
```
SERVER_LOGGING_FORMAT=$'%(asctime)s\t[%(name)25.25s]\t%(levelname)5s: %(message)s'
```
...
1. Use `export SERVER_LOGGING_FORMAT=$'%(asctime)s\t[%(name)25.25s]\t%(levelname)5s: %(message)s'` syntax for local
