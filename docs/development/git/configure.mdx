...
<Note>
    If you'd like to use Mage's GitHub integration (recommended for Github users), there's a different method of configuration. See [this](/production/data-sync/github) page for more information.
</Note>

If you're wondering which Git resource to use in Mage, see [this](docs/getting-started/setting-up-git) page for a high-level overview of Git functionality. If you've decided on a method that uses HTTPS or SSH authentication (Git Sync or Git Integration), this page will walk you through configuring one of those methods.
...
<Step title="[Optional] Enable One-way Git Sync">
    _If_ using Git Sync, tic the box labeld "use One-way Git Sync". You can read more about Git Sync [here](/production/data-sync/git-sync). If One-way Git Sync is enabled, you'll also need to provide:
