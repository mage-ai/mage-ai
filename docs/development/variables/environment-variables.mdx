...
| `MAGE_CODEX_CHAT_MAGE_API_KEY_SECRET` | Mage secret name that stores the API key for AI Chat runner callbacks into the current Mage API.                                                         | `MAGE_API_KEY`                                                           |
...
| `MAGE_CODEX_CHAT_MAX_CONCURRENT_RUNS` | Maximum number of Codex chat turns that can run at the same time in one Mage process. Leave unset or set to `0` for unlimited.                              | `4`                                                                      |
