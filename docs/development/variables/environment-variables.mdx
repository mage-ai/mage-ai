...
| `MAGE_CODEX_CHAT_ATTACHMENT_MAX_BYTES` | Maximum size, in bytes, for one Mage Pro AI Chat attachment.                                                                                             | `10485760`                                                               |
| `MAGE_CODEX_CHAT_ATTACHMENT_MAX_COUNT` | Maximum number of attachments allowed in one Mage Pro AI Chat session.                                                                                   | `20`                                                                     |
| `MAGE_CODEX_CHAT_ATTACHMENT_MAX_SESSION_BYTES` | Maximum total attachment size, in bytes, allowed in one Mage Pro AI Chat session.                                                                  | `52428800`                                                               |
| `MAGE_CODEX_CHAT_MAGE_API_KEY`   | API key the Mage Pro AI Chat runner passes to Mage agent calls back into the current Mage API. Falls back to `MAGE_AGENT_CODEX_API_KEY` or `MAGE_API_KEY`.      | `your_api_key`                                                           |
| `MAGE_CODEX_CHAT_MAGE_API_KEY_SECRET` | Optional Mage secret name that stores the API key for AI Chat runner callbacks into the current Mage API. No secret name is used by default.              | `MAGE_AGENT_API_KEY`                                                     |
| `MAGE_CODEX_CHAT_MAGE_API_URL`   | Base Mage API URL the Mage Pro AI Chat runner gives to the Mage agent MCP server. Falls back to `MAGE_CODEX_CHAT_CLUSTER_URL`, local server settings, or the cluster URL. | `https://your-mage-url`                                           |
| `MAGE_CODEX_CHAT_MAX_CONCURRENT_RUNS` | Maximum number of Codex chat turns that can run at the same time in one Mage process. Unset, empty, invalid, or non-positive values mean unlimited.         | `0` (unlimited), `4`                                                     |
| `MAGE_CODEX_CHAT_MODEL`          | Default model used by Mage Pro AI Chat when a chat request does not override it.                                                                                | `gpt-5.5`                                                                |
| `MAGE_CODEX_CHAT_OPENAI_API_KEY` | OpenAI or Codex API key used by Mage Pro AI Chat direct model authentication. `OPENAI_API_KEY` is also accepted as a fallback.                                  | `your_openai_api_key`                                                    |
| `MAGE_CODEX_CHAT_OPENAI_API_KEY_SECRET` | Mage secret name that stores the API key for Mage Pro AI Chat direct model authentication.                                                              | `OPENAI_API_KEY`                                                         |
| `MAGE_CODEX_CHAT_REASONING_EFFORT` | Default reasoning effort used by Mage Pro AI Chat when a chat request does not override it.                                                                  | `low`, `medium`, `high`, `xhigh`                                         |
| `MAGE_CODEX_CHAT_SERVICE_TIER`   | Default service tier used by Mage Pro AI Chat when a chat request does not override it.                                                                         | `priority`                                                               |
| `MAGE_CODEX_MODEL_AUTH_MODE`     | Select direct API-key authentication or Mage Pro proxy authentication for the AI Chat Codex model provider.                                                     | `direct`, `proxy`                                                        |
| `MAGE_CODEX_MODEL_PROXY_BASE_URL` | OpenAI-compatible proxy base URL used when `MAGE_CODEX_MODEL_AUTH_MODE=proxy`. Defaults from the Mage Pro API endpoint when available.                         | `https://pro-api.mage.ai/api/v1/codex/openai/v1`                         |
...
| `MAGE_PRO_API_API_KEY`           | Mage Pro API key used when Mage Pro proxy mode or other Mage Pro API integrations call the shared Mage Pro API service.                                        | `your_mage_pro_api_key`                                                  |
| `MAGE_PRO_API_ENDPOINT`          | Mage Pro API host used by Mage Pro proxy mode and other Mage Pro API integrations. Do not include the URL scheme. Hosted production clusters use `pro-api.mage.ai`; staging hosts such as `pro-api-staging.mage.ai` are for non-production environments. | `pro-api.mage.ai`                                                        |
