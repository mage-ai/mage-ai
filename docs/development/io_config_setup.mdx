...
version: 0.1.1
default:
  POSTGRES_PASSWORD: "{{ doppler_secret_var('POSTGRES_PASSWORD') }}"
  SNOWFLAKE_PASSWORD: "{{ doppler_secret_var('SNOWFLAKE_PASSWORD') }}"
```

To use Doppler, configure the Mage runtime with `DOPPLER_TOKEN`, `DOPPLER_PROJECT`, and `DOPPLER_CONFIG`. Mage caches Doppler secrets in memory for `DOPPLER_SECRETS_CACHE_TTL_SECONDS` seconds, defaulting to `10`. Set the TTL to `0` to refresh on every lookup. Secret changes are picked up on the next config render after the TTL expires; already-open connections may continue using the previous value until recreated.
...
Profiles must be YAML mappings with string keys. A profile can be at most 1 MiB, and the full `io_config.yaml` file can be at most 10 MiB.
