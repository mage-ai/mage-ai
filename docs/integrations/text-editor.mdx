...
<ParamField path="concurrency_config" type="object">
  ...
  - `pipeline_run_priority` (Mage Pro only): Higher values start first when pipeline run limits are reached.
...
<ParamField path="tags" type="array of strings">
  Free-form labels to organize and search for pipelines. Each tag name can contain up to 70
  characters.
</ParamField>
