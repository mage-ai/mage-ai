---
title: "DBT integration"
---

Transform DBT with Mage.

![](/images/dbt-integration.gif)

1. [Overview](#overview)
2. [Features](#features)
3. [Tutorials](#tutorials)
4. [Guides](#guides)

---

## Overview

Build, run, and manage your DBT models with Mage.

![](/images/dbt-integration-2.gif)

| Role                                   | Benefits                                                                                                                                                                       |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Analytics engineers<img width="100" /> | With Mage and DBT combined, you can expand build and run complex data pipelines.                                                                                               |
| Data engineers<img width="100" />      | Simplify your data infrastructure and reduce the amount of redundant tools in your “modern” data stack. Replace DBT cloud and Airflow with Mage’s native integration with DBT. |

### Prerequisites

To use the DBT integration, you must run Mage using
[Docker](getting-started/setup#using-docker).

---

## Features

| Feature                                                           | Description                                                                                                           |
| ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| Schedule DBT model runs                                           | Trigger your DBT model runs on a regular schedule, from an event, or via API request.                                 |
| Run specific DBT models and their dependencies                    | ![](https://github.com/mage-ai/assets/blob/main/dbt/add-dbt-model.gif?raw=true)                                       |
| Run all models and optionally exclude others                      | ![](https://raw.githubusercontent.com/mage-ai/assets/main/dbt/add-dbt-models.gif)                                     |
| DBT models can depend on non-DBT related tasks                    | _Examples:_<br />Build model after data ingestion from API<br />Build model after another pipeline completes          |
| Preview DBT model results as you write SQL                        | ![](https://github.com/mage-ai/assets/blob/main/dbt/dbt-preview.gif?raw=true)                                         |
| Build dynamic DBT pipelines using flexible variable interpolation | `{{ env_var('...') }}`<br />`{{ variables('...') }}`                                                                  |
| Automatically run DBT tests every time a pipeline runs            | Write checks using DBT tests, then Mage will run them and fail the pipeline if the test results produce any failures. |
| Observability built-in                                            | Monitor your DBT pipelines and get alerted when things break.                                                         |

---

## Tutorials

- [Set up DBT models & orchestrate DBT runs](https://github.com/mage-ai/mage-ai/blob/master/docs/tutorials/dbt/quick_start.md)

---

## Guides

- [Run a single model](/integrations/run-single-model)
- [Run selected models (and optionally exclude others)](/integrations/run-selected-model)
- [Connection profiles](https://github.com/mage-ai/mage-ai/blob/master/docs/guides/dbt/connection_profiles.md)
- [DBT models depending on other blocks using sources](https://github.com/mage-ai/mage-ai/blob/master/docs/guides/dbt/dependencies.md)
- [Running DBT tests](https://github.com/mage-ai/mage-ai/blob/master/docs/guides/dbt/tests.md)
- [Add an existing DBT project to Mage](https://github.com/mage-ai/mage-ai/blob/master/docs/guides/dbt/add_existing_project.md)
