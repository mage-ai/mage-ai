...
For a **remote Spark cluster** (e.g. Spark Thrift server or cluster URL), set `SPARK_HOST` to the master URL (e.g. `spark://host:7077` or `local[*]`). You can also set optional keys such as `SPARK_PORT`, `SPARK_TOKEN`, `SPARK_USER`, `SPARK_DRIVER`, `SPARK_CLUSTER`, `SPARK_ENDPOINT`, `SPARK_ORGANIZATION`, `SPARK_SERVER_SIDE_PARAMETERS` as needed for your deployment.
