...
To overwrite a column type when running a python export block, simply specify the column name and type in the `overwrite_type` dict in data exporter config
...
def export_data_to_mssql(df: DataFrame, **kwargs) -> None:
    ...
    config_path = path.join(get_repo_path(), 'io_config.yaml')
    config_profile = 'default'
    overwrite_type = {'column_name': 'VARCHAR(255)'}
