...
```bash
docker run -it -n mage_databricks -p 6789:6789 -v $(pwd):/home/src mageai_databricks \
...
Change the value for key `remote_variables_dir` to be a remote S3 bucket that
