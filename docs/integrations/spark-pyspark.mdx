...
When Mage pod is running, you can create a "Standard (batch)" pipeline and then write PySpark code in any blocks.
...
When Mage is running, you can create a "Standard (batch)" pipeline and then write PySpark code in any blocks.
...
### 3. Review the `spark_config` settings in the `metadata.yaml` file under
the pipeline folder, and make necessary adjustments
...
### 4. Run PySpark code in Mage
When Mage is running, you can create a "Standard (batch)" pipeline and then write
PySpark code using the Spark session from the `kwargs` via `kwargs['spark']`, e.g.,
...
```python
from mage_ai.data_preparation.models.block.utils import get_spark_session
