...
```bash
mage create-spark-cluster demo_project
```
...
Here is an overview of the steps required to use Mage locally with Spark in Azure HDInsight:

---

1. [Create an SSH public-private key pair](#1-create-an-ssh-public-private-key-pair)
2. [Create a Blob Storage for Spark](#2-create-a-blog-storage-for-spark)
3. [Start Mage for Azure](#3-start-mage-for-azure)
4. [Configure project’s HDInsight metadata settings](#4-configure-projects-hdinsight-metadata-settings)
5. [Launch HDInsight cluster](#5-launch-emr-cluster)
6. [Allow HDInsight connection permissions](#6-allow-emr-connection-permissions)
7. [Sample pipeline with PySpark code on HDInsight](#7-sample-pipeline-with-pyspark-code-on-hdinsight)
8. [Debugging PySpark in Azure HDInsight](#8-debugging-pyspark-in-azure-hdinsight)
9. [Clean up the HDInsight cluster](#9-clean-up-the-hdinsight-cluster)
