...
<Warning>
  Event triggers are deprecated for new trigger creation. Use API triggers for new event-driven
  and webhook-style workflows. Existing legacy event triggers may still appear in older projects.
</Warning>
...
The new trigger form opens as a draft. Mage does not create the trigger until you click
**Create trigger**, and it warns you before leaving the page if the draft has unsaved changes.
...
- **Disable all triggers in Mage Pro**: On the global Triggers page, the current search and filters
  define the scope; only matching active triggers are disabled
...
- **Mage Pro**: Right-click a trigger row to open its context menu. Select **Copy URL** to copy the
  trigger's direct URL, or select **Delete** and confirm **Yes** to delete the trigger.
