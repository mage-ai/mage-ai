...
- If a block is selected in an integration pipeline to retry block runs, only the block
runs for the selected block's stream will be ran (e.g. If there are two streams--Stream 1
and Stream 2--and we select to retry runs from the transformer block for Stream 1, the
transformer and data loader blocks for Stream 1 will be re-ran. No blocks will be re-ran
for Stream 2).
