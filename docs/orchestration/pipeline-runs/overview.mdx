...
6. **Review Kubernetes resource usage** in the **Resources** tab when stored metrics are available for Mage Pro runs that use the Kubernetes executor
...
- **Kubernetes resource summary** - In Mage Pro, review stored CPU and memory usage plus copied block-level executor recommendations for runs that use the Kubernetes executor
