...
3. **Filter** by pipeline, status, or date range, and **search** by pipeline or trigger
...
- **Date range** - Filter current and past runs by bounded UTC preset ranges (Last hour, Last day, Last week, Last 30 days, Last year), all time, or a custom UTC start and end date
- **Search** - Text search by pipeline or trigger (Mage Pro only)
...
In Mage Pro, right-click a run on either the project-wide **Pipeline runs** page or a
pipeline's **Runs** page to open the run actions menu. From this menu, you can open the run,
edit its pipeline, view its logs, select **Copy URL** to copy its direct URL, select **Retry
pipeline run** to restart the entire run, or select **Retry incomplete blocks** on a failed run
to resume from its incomplete blocks.
**Cancel** is enabled only for running runs, and **Fix with AI** is enabled only for failed
runs when AI Sidekick is enabled; unavailable actions remain visible but disabled.
...
6. **Review observed Kubernetes resource usage** in the **Resources** tab when stored average and peak CPU/RAM metrics are available for Mage Pro runs that use the Kubernetes executor. Use pipeline **Monitoring** for pipeline-level Kubernetes executor recommendations.

In Mage Pro, right-click a block run to open its block in the pipeline editor, view its logs,
or retry it after the pipeline run finishes. When CSV output download is available for a
completed block run, the menu also includes **Download output**.
...
Select an incomplete stream row in a data integration pipeline run to retry only that stream. The broader retry incomplete action remains available for retrying every incomplete stream. For detailed instructions, see [Retrying block runs from a pipeline run](/orchestration/pipeline-runs/retrying-block-runs).
...
3. **Open the log source** from the selected error log's source-location link, or click the block name in the Block column, to open the failing block in the pipeline editor. When line metadata is available, Mage focuses that line in the editor.
4. **Review block output** to understand data state at failure
5. **Check upstream dependencies** to ensure data flow is correct
...
- **Kubernetes resource summary** - In Mage Pro, review observed average and peak CPU/RAM for Kubernetes executor runs; use pipeline Monitoring for pipeline-level Kubernetes executor recommendations
