...
Because `pill=red`, the `red_pill` block will execute, and the `blue_pill` block will not. Additionally, anyblocks _downstream_ of the `blue_pill` block will not execute.
...
You can also interpolate variables, both through `kwargs` and other variables— see our [docs](/development/variables/overview) for more info.
