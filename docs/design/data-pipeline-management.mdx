...
* `pipeline_run_priority` (Mage Pro only): set the priority for this pipeline's runs when run limits are reached.
  Higher values start first, and runs with the same priority use execution date order.
  The default value is `0`. You can edit this in Pipeline Settings as **Pipeline priority**.
