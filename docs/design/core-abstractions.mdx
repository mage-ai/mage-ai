...
New trigger creation supports 2 trigger types:

1. Schedule

2. API

Event triggers are deprecated and may only appear for existing legacy
configurations.
...
Event triggers are deprecated for new trigger creation. Use API triggers for new
event-driven or webhook-style workflows. Existing legacy event triggers may
still appear in older projects.
