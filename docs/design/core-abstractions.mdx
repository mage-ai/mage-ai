...
- The `exensions` folder is used for Mage extensions that integrate other data tools, like [Great Expectations](/development/testing/great-expectations).
...
`mage_data` holds project-level cache data and stores the result of Block runs, which are then returned to the user via the UI.


## Pipeline

A pipeline contains references to all the blocks of code you want to run, charts
for visualizing data, and organizes the dependency between each block of code.

Each pipeline is represented by a YAML file. Here is an
[example](https://github.com/mage-ai/demo%5Fetl%5Fpipeline/blob/master/pipelines/etl%5Fdemo/metadata.yaml).
