  ...
  - The SQLite DB stores project-level metadata, such as pipeline and block run data. Here's a sample tree tables in the `tables` schema:
   ```
    └── tables
        ...
        └── user_role
    ```
  - You can connect to the SQLite DB like any other database. The JDBC URL is `/home/src/mage_data/[project_folder]/mage-ai.db` by default for Docker installs or `~/.mage_data/[project_folder]/mage-ai.db` for pip installs.
