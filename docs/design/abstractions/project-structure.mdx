...
A Mage project lives inside it's own folder. It's there where mage data is stored (`mage_data`) and individual projects are housed. You can think of a project as an environment: it contains configurations, pipelines, and other project-specific data. A [workspace](production/developing-in-the-cloud/workspaces/overview) can hold multiple projects.
