...
> **Note:** UPSERT (UPDATE or INSERT) functionality is only supported on a subset of SQL databases (e.g., PostgreSQL, MySQL, MSSQL, Snowflake, BigQuery, Databricks SQL, SQLite). It is **not** supported on Redshift, ClickHouse, or Trino. See below for details.
