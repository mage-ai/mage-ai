...
Handles data transfer between an Azure Blob Storage conatiner and the Mage app. Supports loading files of any of the following types:
-   CSV
-   JSON
-   Parquet
-   HDF5

To authenticate (and authorize) access to Azure Blob Storage, the following environment variables must be set:
...
-   **Args**:
    ...
    -   `df (DataFrame)`: Data frame to export.
    -   `container_name (str)`: Name of the container to export data to.
