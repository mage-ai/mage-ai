...
icon: "code"
description: "Connect Mage Pro to GitHub, Bitbucket, GitLab, or Azure DevOps, manage code with Git, and deploy from a branch or fixed release tag."
---
...
2. **Deployments** — [Deploy](#2-deployments-app) a commit from a branch or a fixed release tag to your Mage Pro cluster or workspace from the visual Deployments app.
...
If you change authentication type, **save authentication settings** before continuing with the rest of the connection configuration. Switching types clears the credentials from the other method in your deployment settings. For **HTTPS + PAT** and **SSH**, run **Test connection** after saving. A successful test enables deployment selection from the Deployments list; if the repository URL, username, email, token, or SSH keys change later, Mage marks the connection as needing another test.
...
6. Click **Test connection**. After the test passes, Mage shows **Verified for deployment selection** and the Deployments list can show **Select deployment** for this configuration.
...
### Choose a branch or release tag

In **Connect repository** → **Production source**, choose the Git reference your production deployment should use:

- **Branch** — select a branch, then choose from the commits available on that branch. Use this for a moving source that follows your normal continuous delivery workflow.
- **Tag** — select an available Git tag as a fixed release version. Use this when production should point to a named release boundary instead of a moving branch.

After choosing the source type, click **Select version** to choose the exact commit or tag to deploy. The repository, source type, and selected version remain visible in the deployment configuration so reviewers and operators can identify what production is running.

<Frame>
  <img
    alt="Mage Pro production source setup with Tag selected"
    src="/media/deployments/release-tag-production-source.png"
  />
</Frame>
...
If a configuration that uses **HTTPS + PAT** or **SSH** shows **Test connection required** instead of **Select deployment**, open **Configure**, run **Test connection**, and return to the Deployments list after the test passes. Mage requires a fresh successful test after saved custom authentication settings change.
