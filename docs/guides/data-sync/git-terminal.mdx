...
Terminal sessions load Mage-managed safeguards for AI and Mage API access. Codex
commands launched from the terminal use Mage Pro proxy authentication instead of
provider API keys inherited by the shell. Direct shell calls from the browser
terminal to Mage REST endpoints such as pipelines, pipeline runs, blocks, block
runs, pipeline schedules, and status are blocked; use the Mage UI, the
documented API from outside the workspace terminal, or Mage agent MCP tools for
those operations.
