...
The Mage Pro image includes common Git provider CLIs for terminal workflows: GitHub CLI (`gh`), GitLab CLI (`glab`), Azure CLI (`az`) with the Azure DevOps extension, and the community Bitbucket CLI (`bitbucket-cli`).
...
Terminal sessions load Mage-managed safeguards for AI and Mage API access. Direct
shell calls from the browser terminal to Mage REST endpoints such as pipelines,
pipeline runs, blocks, block runs, pipeline schedules, and status are blocked;
use the Mage UI, the documented API from outside the workspace terminal, or Mage
agent MCP tools for those operations.
