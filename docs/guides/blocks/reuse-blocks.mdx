...
<ProOnly source="block-templates" />
...
5. **Select template** - Browse or search the folders, then select a template to preview its code
6. **Add to workflow** - Click **Add to workflow**. Core templates are added directly; Team templates open the add-block form so you can enter a block name, then click **Save and add**.

The **dbt** folder contains three workflow starters: select a model or snapshot
from a file, run all models with optional selection or exclusion rules, or
start with a generic dbt command. Selecting the file-based starter opens the
file browser so you can choose the model or snapshot to add.
...
When you delete a shared block from the pipeline editor, choose whether to remove only the pipeline reference or remove the block file too:

- **Remove from this pipeline only** - Removes the block from the current pipeline and keeps the shared block file available to other pipelines.
- **Remove and delete the block file** - Permanently deletes the block file only when the block is not a replica and no other pipeline uses that file. This option is disabled when deleting the file would break another pipeline or replica.
