...
You can also use the output from an upstream Mage block inside a dbt SQL model
with `block_output`. This is useful when a Python, SQL, or dynamic block decides
...
def load_data(*args, **kwargs):
    return [
        {
            'city': 'seattle',
            'run_label': 'daily_refresh',
        },
    ]
```

Then a dbt SQL model block can interpolate that output before dbt runs:

```sql
select
  '{{ block_output("select_city", parse=lambda rows: rows[0]["city"]) }}' as city,
  '{{ block_output("select_city", parse=lambda rows: rows[0]["run_label"]) }}' as run_label
```

If the upstream block is dynamic, Mage resolves the output for the current
dynamic run and passes that value to the dbt model. For structured outputs, use
`parse` to return a scalar value that is valid SQL.

### Run one dbt model for each dynamic output

For example, create a data loader named `dynamic_cities` that returns one item
for each dbt model run:

```python
@data_loader
def load_dynamic_cities(*args, **kwargs):
    outputs = [
        [{'city': 'seattle'}],
        [{'city': 'portland'}],
    ]
...
In the Pipeline Editor, mark `dynamic_cities` as a dynamic block. Then add a dbt
SQL model directly downstream from it:

```sql
select
  '{{ block_output(
      "dynamic_cities",
      parse=lambda rows: rows[0]["city"]
  ) }}' as city
```

When the pipeline runs, Mage creates two dynamic runs of the downstream dbt
block. The first run receives the output at index `0` and executes SQL containing
`select 'seattle' as city`. The second receives index `1` and executes SQL
containing `select 'portland' as city`.

These are dynamically created block runs, not copies created with the manual
block replication feature. The optional metadata entries can include
`block_uuid` values when you want stable, descriptive identifiers for the runs.
