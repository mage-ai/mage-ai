...
For supported SQL sources, the selected stream configuration surface also shows
**Count rows** next to **Load sample data**. Click **Count rows** after selecting
and configuring a stream to count only that active stream on demand. Mage shows
one context-appropriate result: `FULL_TABLE` streams show the total row count,
and `INCREMENTAL` streams show the row count eligible after the saved bookmark.
If no bookmark exists, Mage warns that the next incremental sync may read all
rows. Log-based SQL replication streams do not support row counts. Slow or
transient failed count requests show an error for the active stream and can be
retried without automatically counting every stream. Permission failures show
the source error without a retry action, since the source credentials or grants
must be fixed first.
...
Once you’re done configuring your pipeline, open the pipeline’s **Triggers**
page from the pipeline navigation.

While you’re still in the editor, the breadcrumbs in your header could look like this:
`Pipelines / pipeline name / Editor`.
