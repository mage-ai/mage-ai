...
1. Enter your trigger configs into the `triggers.yaml` file.
    ...
    1. Optional trigger fields
        ...
        1. `envs`: The environments that the trigger runs in. If envs is not set or empty, the trigger will run in all environments. The environment of the Mage app is set via `ENV` [environment variable](/development/environment-variables).
