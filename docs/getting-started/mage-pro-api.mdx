...
API runs execute pipelines from the current Mage repository. The legacy `project` option is no
longer supported; send API-run requests to the workspace that owns the pipeline instead.
