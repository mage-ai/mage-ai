...
<AccordionGroup>
<Accordion title="Docker template" icon="Docker">

Be sure that Docker and Git are installed and run:

```bash
git clone github.com/mage-ai/mage-template mage-quickstart \
&& cd mage-quickstart \
&& docker compose up
```
...
See [here](/development/variables/environment-variables) for a list of environment variables you can configure.

### Setting prefix for Mage url

In some cases, you may need to add a prefix to the Mage url. You can configure the Mage server to start
...
this [document](/contributing/overview).
