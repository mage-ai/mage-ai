...
1. Open your [GitLab profile settings](https://gitlab.com/-/profile).
2. On the left hand side, click the section called **SSH keys**.
3. In the text area labeled **Key**, paste in the output text you copied after
   running the command `cat ~/.ssh/id_rsa.pub` in your terminal.
4. Enter in anything into the input field labeled **Title**. For example,
   `mage-cloud-key`.
5. Under the dropdown labeled **Usage type**, select the `Authentication`
   option.
