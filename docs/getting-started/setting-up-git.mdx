...
There three different ways to use Git in Mage and _four_ ways to version control your files— we'll discussing each of them. We should note that the thre ways of using Git are mutually exclusive. That means that you can only choose one of the three options. However, you can use any of the four version control methods in conjunction with any of the three Git methods.
...
Git Sync is particularly useful in production instanaces, where other Git methods are can be used more meaningfully in development instances. 

Since Git sync is a one-way sync, it's not very useful for collaboration, since changes get overridden. However, this makes it great for a production Mage instance, where verified changes should only be accepted from development environments.

A common pattern is to use devevelopment instances to edit and push (Git integration), then have production instances use Git Sync after CI/CD.
