...
from mage_integrations.sources.doris import Doris


def build_sample_doris_rows():
    return [
...
class DorisSourceTests(unittest.TestCase):
    maxDiff = None

    def test_discover(self):
        source = Doris(config=dict(database="demo_db"))
        doris_connection = MagicMock()
        doris_connection.load.return_value = build_sample_doris_rows()
        with patch.object(source, "build_discover_query") as mock_build_query:
            with patch.object(
                source,
                "build_connection",
                return_value=doris_connection,
