...
WHERE TABLE_NAME = '{table_name}' AND TABLE_SCHEMA = '{schema_name}'
    ...
    def build_insert_commands(
        ...
        if unique_constraints and UNIQUE_CONFLICT_METHOD_UPDATE == unique_conflict_method:
            ...
            create_temp_table_command = (f'CREATE TABLE {full_table_name_temp} AS '
                                         f'SELECT * FROM ( SELECT * FROM {full_table_name}) WHERE 1=0')
            ...
            delete_records_from_full_table = f'DELETE FROM {full_table_name} where ({", ".join(unique_constraints_clean)}) '
            f'in (SELECT {", ".join(unique_constraints_clean)} from {full_table_name_temp})'
            ...
            # This is temp as need to know the best way to create table programmatically */
            commands = [create_temp_table_command] + commands + [
