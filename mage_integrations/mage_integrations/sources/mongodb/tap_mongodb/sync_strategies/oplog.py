...
def sync_collection(client, stream, state, stream_projection, max_oplog_ts=None):
    ...
    activate_version_message = singer.ActivateVersionMessage(
        stream=common.calculate_destination_stream_name(stream), version=version
    )
    ...
    with client.local.oplog.rs.find(
        oplog_query, projection, sort=[('$natural', pymongo.ASCENDING)], oplog_replay=oplog_replay
    ) as cursor:
        for row in cursor:
            ...
            if row.get('ts') and row.get('ts') < timestamp.Timestamp(
                stream_state['oplog_ts_time'], stream_state['oplog_ts_inc']
            ):
            ...
            if len(update_buffer) >= MAX_UPDATE_BUFFER_LENGTH:
                for buffered_row in flush_buffer(
                    client, update_buffer, stream_projection, database_name, collection_name
                ):
