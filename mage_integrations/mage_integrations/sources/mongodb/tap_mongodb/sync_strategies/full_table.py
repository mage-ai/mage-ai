...
def sync_collection(client, stream, state, projection):
    ...
    was_interrupted = (
        singer.get_bookmark(state, stream['tap_stream_id'], 'last_id_fetched') is not None
    )
    ...
    activate_version_message = singer.ActivateVersionMessage(
        stream=common.calculate_destination_stream_name(stream), version=stream_version
    )
    ...
    if max_id_value:
        ...
        state = singer.write_bookmark(
            ...
            'max_id_value',
            common.class_to_string(max_id_value, max_id_value.__class__.__name__),
        )
        state = singer.write_bookmark(
            state, stream['tap_stream_id'], 'max_id_type', max_id_value.__class__.__name__
        )

    find_filter = {'$lte': max_id_value}
    if last_id_fetched:
        last_id_fetched_type = singer.get_bookmark(
            state, stream['tap_stream_id'], 'last_id_fetched_type'
        )
    ...
    with collection.find(
        {'_id': find_filter}, projection, sort=[("_id", pymongo.ASCENDING)]
    ) as cursor:
        rows_saved = 0
