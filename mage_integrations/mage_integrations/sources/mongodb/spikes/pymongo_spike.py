...
client = pymongo.MongoClient(
    host=host, username=username, password=password, port=27017, authSource=auth_source, ssl=ssl
)
...
sources_team_members_coll.insert_many(
    [
        {"name": "Jacob", "membersince": 2019, "my_object": {"nested_field": "some_value"}},
...
update_result = sources_team_members_coll.update_one(
    {"name": "Nick"}, {"$set": {"membersince": 2017}}
)
for doc in sources_team_members_coll.find():
    print(doc)
...
if should_print_oplog:
    ...
    with client.local.oplog.rs.find({'ts': {'$gt': ts}}, oplog_replay=True) as cursor:
        for row in cursor:
            if row['op'] in ['i', 'u', 'd']:
