    ...
    def test_run(self):
        ...
        for tap_stream_id in self.expected_sync_streams():
            self.assertGreaterEqual(
                record_count_by_stream[tap_stream_id], self.expected_row_count_1()[tap_stream_id]
            )
        ...
        with get_test_connection() as client:
            ...
            docs = list(
                client.local.oplog.rs.find(sort=[('$natural', pymongo.DESCENDING)]).limit(20)
            )
            ...
            interrupted_state['bookmarks']['simple_db-' + table_interrupted].update(
                {'oplog_ts_time': final_result[0]}
            )
            interrupted_state['bookmarks']['simple_db-' + table_interrupted].update(
                {'oplog_ts_inc': final_result[1]}
            )
            ...
            client["simple_db"]["simple_coll_1"].find_one_and_update(
                {"_id": doc_to_update_1["_id"]}, {"$set": {"int_field": 999}}
            )
            ...
            last_inserted_coll_1 = client["simple_db"]["simple_coll_1"].insert_many(
                generate_simple_coll_docs(1)
            )
            ...
            last_inserted_coll_3 = client["simple_db"]["simple_coll_3"].insert_many(
                generate_simple_coll_docs(1)
            )
        ...
        self.assertGreater(
            record_count_by_stream[table_interrupted], record_count_by_stream_2[table_interrupted]
        )
        ...
        for tap_stream_id in initial_state['bookmarks'].keys():
            ...
            self.assertSetEqual(
                set(initial_state['bookmarks'][tap_stream_id].keys()),
                set(second_state['bookmarks'][tap_stream_id].keys()),
            )

            # check the table versions is the same
            self.assertEqual(
                second_state['bookmarks'][tap_stream_id]['version'],
                initial_state['bookmarks'][tap_stream_id]['version'],
            )

            # verify the method has not changed
            self.assertEqual(
                initial_state['bookmarks'][tap_stream_id]['last_replication_method'],
            ...
            self.assertGreater(
                second_state['bookmarks'][tap_stream_id]['oplog_ts_time'],
        ...
        for tap_stream_id in expected_sync_streams:
            self.assertEqual(
                record_count_by_stream_3[tap_stream_id], expected_row_count_3[tap_stream_id]
            )
        ...
        self.assertEqual(
            records_by_stream_3['simple_coll_3']['messages'][0]['action'], 'activate_version'
        )
        self.assertEqual(records_by_stream_3['simple_coll_3']['messages'][1]['action'], 'upsert')
        self.assertEqual(
            records_by_stream_3['simple_coll_3']['messages'][1]['data']['_id'],
            last_inserted_id_coll_3,
        )
        ...
        for tap_stream_id in third_state['bookmarks'].keys():
            # verify the bookmark keys have not changed in a resumed sync
            self.assertSetEqual(
                set(third_state['bookmarks'][tap_stream_id].keys()),
                set(second_state['bookmarks'][tap_stream_id].keys()),
            )
            ...
            self.assertEqual(
                second_state['bookmarks'][tap_stream_id]['version'],
                third_state['bookmarks'][tap_stream_id]['version'],
            )

            # verify the method has not changed
            self.assertEqual(
                third_state['bookmarks'][tap_stream_id]['last_replication_method'],
                second_state['bookmarks'][tap_stream_id]['last_replication_method'],
            )
