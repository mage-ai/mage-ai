...
class MongoDBOplogAgedOut(unittest.TestCase):
    ...
    def test_run(self):
        ...
        for stream_name in self.expected_sync_streams():
            ...
            self.assertEqual(
                'activate_version', records_by_stream[stream_name]['messages'][51]['action']
            )
        ...
        self.assertNotEqual(
            original_version,
            final_state.get('bookmarks', {}).get('simple_db-simple_coll_1', {}).get('version'),
        )
        ...
        self.assertIsNotNone(
            final_state.get('bookmarks', {}).get('simple_db-simple_coll_1', {}).get('oplog_ts_time')
        )
        self.assertIsNotNone(
            final_state.get('bookmarks', {}).get('simple_db-simple_coll_1', {}).get('oplog_ts_inc')
        )
        self.assertTrue(
