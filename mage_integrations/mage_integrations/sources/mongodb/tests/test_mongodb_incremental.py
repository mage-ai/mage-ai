...
class MongoDBIncremental(TestCase):
    ...
    def expected_incremental_int_fields(self):
        return {
            ...
            **{
                "simple_coll_{}".format(k): {49, 50, 51, 0}
                for k in self.key_names()
                if k not in ("simple_coll_int_field")
            },
    ...
    def test_run(self):
        ...
        with get_test_connection() as client:
            ...
            for key_name in self.key_names():
                if key_name == 'int_field':
                    ...
                    doc_to_update = client["simple_db"]["simple_coll_{}".format(key_name)].find_one(
                        sort=[("{}".format(key_name), -1)]
                    )
                    value = doc_to_update["{}".format(key_name)]
                    int_based_coll = client["simple_db"][
                        "simple_coll_{}".format(key_name)
                    ].find_one()
                    client["simple_db"]["simple_coll_{}".format(key_name)].find_one_and_update(
                        {"_id": int_based_coll["_id"]}, {"$set": {"{}".format(key_name): value + 3}}
                    )
                    ...
                    value = doc_to_update["{}".format(key_name)]
                    double_based_coll = client["simple_db"][
                        "simple_coll_{}".format(key_name)
                    ].find_one()
                    client["simple_db"]["simple_coll_{}".format(key_name)].find_one_and_update(
                        {"_id": double_based_coll["_id"]},
                        {"$set": {"{}".format(key_name): value + 3}},
                    )
                    ...
                    value = doc_to_update["{}".format(key_name)]
                    bit64_int_based_coll = client["simple_db"][
                        "simple_coll_{}".format(key_name)
                    ].find_one()
                    client["simple_db"]["simple_coll_{}".format(key_name)].find_one_and_update(
                        {"_id": bit64_int_based_coll["_id"]},
                        {"$set": {"{}".format(key_name): value + 3}},
                    )
                elif key_name == 'date_field':
                    date_based_coll = client["simple_db"][
                        "simple_coll_{}".format(key_name)
                    ].find_one()
                    client["simple_db"]["simple_coll_{}".format(key_name)].find_one_and_update(
                        {"_id": date_based_coll["_id"]},
                        {"$set": {"{}".format(key_name): datetime(2021, 1, 1, 15, 30, 14, 222000)}},
                    )
                elif key_name == 'timestamp_field':
                    timestamp_based_coll = client["simple_db"][
                        "simple_coll_{}".format(key_name)
                    ].find_one()
                    client["simple_db"]["simple_coll_{}".format(key_name)].find_one_and_update(
                        {"_id": timestamp_based_coll["_id"]},
                        {
                            "$set": {
                                "{}".format(key_name): bson.timestamp.Timestamp(1565897157 + 99, 1)
                            }
                        },
                    )
        ...
        client["simple_db"]["simple_coll_1"].find_one_and_update(
            {"_id": no_rep_doc_coll_1["_id"]}, {"$set": {"string_field": 'No_replication'}}
        )
        ...
        for stream_catalog in found_catalogs:
            ...
            if stream_catalog['tap_stream_id'] == 'simple_db-simple_coll_1':
                additional_md = [
                    {"breadcrumb": [], "metadata": {'replication-method': 'LOG_BASED'}}
                ]
                ...
                additional_md = [
                    {
                        ...
                        "metadata": {
                            'replication-method': 'INCREMENTAL',
                            'replication-key': stream_catalog['stream_name'].replace(
                                'simple_coll_', ''
                            ),
                        },
                    }
