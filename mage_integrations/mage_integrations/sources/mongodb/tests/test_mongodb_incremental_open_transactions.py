    ...
    def expected_pk_values_3(self):
        return {
            'simple_db_simple_coll_1': [
                9,
                11,
            ],  # bookmarked value = 9 and updated value in test = 11
            'simple_db_simple_coll_2': [
                19,
                21,
            ],  # bookmarked value = 19 and updated value in test = 21
    ...
    def test_run(self):
        ...
        with get_test_connection() as client:
            ...
            client["simple_db"]["simple_coll_1"].update_one(
                {"int_field": 5}, {"$set": {"int_field": 11}}, session=session2
            )
            client["simple_db"]["simple_coll_2"].update_one(
                {"int_field": 10}, {"$set": {"int_field": 21}}, session=session2
            )
            ...
            self.assertEqual(
                self.expected_check_streams_sync_1(), {c['tap_stream_id'] for c in found_catalogs}
            )
            ...
            record_count_by_stream = runner.examine_target_output_file(
                self, conn_id, self.expected_sync_streams_1(), self.expected_pks_1()
            )
            ...
            for stream in self.expected_sync_streams_1():
                records_2[stream] = [
                    x for x in records_by_stream[stream]['messages'] if x.get('action') == 'upsert'
                ]
                pk_2 = []
            ...
            record_count_by_stream_2 = runner.examine_target_output_file(
                self, conn_id, self.expected_sync_streams_2(), self.expected_pks_2()
            )
            ...
            for stream in self.expected_sync_streams_1():
                records_3[stream] = [
                    x
                    for x in records_by_stream_2[stream]['messages']
