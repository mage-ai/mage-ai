...
class MongoDBFieldNameRestrictions(unittest.TestCase):
    ...
    def test_run(self):
        ...
        with get_test_connection() as client:
            ...
            object_id = client['simple_db']['simple_coll_1'].find()[last_index][
                '_id'
            ]  # int.field 49
            changed_ids.add(object_id)
            ...
            object_id = client['simple_db']['simple_coll_2'].find()[last_index][
                '_id'
            ]  # int.field 99
            changed_ids.add(object_id)
        ...
        actual = set(
            [ObjectId(x['data']['_id']) for x in records_by_stream['simple_coll_1']]
        ).union(set([ObjectId(x['data']['_id']) for x in records_by_stream['simple_coll_2']]))
        self.assertEqual(changed_ids, actual)

        # Verify the updated record values in the db match the tap output file
        found_id_db = client['simple_db']['simple_coll_1'].find({'string$field': "Updated_1"})[0][
            '_id'
        ]
        found_id_tap = [
            x['data']['_id']
        ...
        found_id_db = client['simple_db']['simple_coll_2'].find({'string$field': "Updated_2"})[0][
            '_id'
        ]
        found_id_tap = [
            x['data']['_id']
