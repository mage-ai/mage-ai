...
def generate_simple_coll_docs(num_docs):
    ...
    for int_value in range(num_docs):
        docs.append(
            {"_id": int_value, "int_field": int_value, "string_field": random_string_generator()}
        )
    return docs
...
def generate_simple_binary_coll_docs(num_docs):
    ...
    for int_value in range(num_docs):
        docs.append(
            {
                "_id": bson.Binary("test {}".format(int_value).encode()),
                "int_field": int_value,
...
class MongoDBFullTableID(unittest.TestCase):
    ...
    def test_run(self):
        ...
        interrupted_state = {
            ...
            'bookmarks': {
                ...
                'simple_db-simple_coll_2': {
                    'max_id_value': base64.b64encode("test {}".format(49).encode()),
                    'max_id_type': 'bytes',
                    'initial_full_table_complete': False,
        ...
        for stream_name in self.expected_sync_streams():
            self.assertNotEqual(
                'activate_version', records_by_stream[stream_name]['messages'][0]['action']
            )
            self.assertEqual(
                'activate_version', records_by_stream[stream_name]['messages'][-1]['action']
            )
        ...
        self.assertNotEqual(
            999, records_by_stream['simple_coll_1']['messages'][0]['data']['int_field']
        )
        self.assertNotEqual(
            888, records_by_stream['simple_coll_2']['messages'][0]['data']['int_field']
        )
        ...
        self.assertEqual(
            777, records_by_stream['simple_coll_1']['messages'][5]['data']['int_field']
        )
        self.assertEqual(
            666, records_by_stream['simple_coll_2']['messages'][15]['data']['int_field']
        )
