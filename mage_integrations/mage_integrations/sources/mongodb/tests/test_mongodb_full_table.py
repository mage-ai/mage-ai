...
class MongoDBFullTable(unittest.TestCase):
    def setUp(self):
        ...
        with get_test_connection() as client:
            ...
            client["simple_db"]["simple_coll_4"].insert_one(
                {"another_hebrew_ישראל": "another_hebrew_ישרא"}
            )
            nested_doc = {"field0": {}}
    ...
    def test_run(self):
        ...
        with get_test_connection() as client:
            ...
            client["simple_db"]["simple_coll_1"].find_one_and_update(
                {"_id": doc_to_update["_id"]}, {"$set": {"int_field": 999}}
            )

            doc_to_update = client["simple_db"]["simple_coll_2"].find_one()
            client["simple_db"]["simple_coll_2"].find_one_and_update(
                {"_id": doc_to_update["_id"]}, {"$set": {"int_field": 888}}
            )

            doc_to_update = client["admin"]["admin_coll_1"].find_one()
            client["admin"]["admin_coll_1"].find_one_and_update(
                {"_id": doc_to_update["_id"]}, {"$set": {"int_field": 777}}
            )
        ...
        new_expected_row_counts = {
            k: v + 2
            for k, v in self.expected_row_counts().items()
            if k not in ['simple_db_simple_coll_3', 'simple_db_simple_coll_4']
        }
        ...
        for tap_stream_id in self.expected_check_streams():
            ...
            self.assertEqual(
                records_by_stream[self.tap_stream_id_to_stream()[tap_stream_id]]['table_version'],
                second_versions[tap_stream_id],
            )
