...
class Stream:
    ...
    def check_access(self):
        """Check whether the permission was given to access stream resources or
        not."""
...
class CursorBasedExportStream(Stream):
    ...
    def get_objects(self, start_time):
        """Retrieve objects from the incremental exports endpoint using cursor
        based pagination."""
        url = self.endpoint.format(self.config["subdomain"])
        # Pass `request_timeout` parameter
...
def raise_or_log_zenpy_apiexception(schema, stream, e):
    ...
    if (
        ...
        == "You do not have access to this page. Please contact the account owner of this help desk"
        " for further help."
    ):
        LOGGER.warning(
            "The account credentials supplied do not have access to `%s` custom fields.", stream
...
class Organizations(Stream):
    ...
    def check_access(self):
        """Check whether the permission was given to access stream resources or
        not."""
        # Convert datetime object to standard format with timezone. Used utcnow to reduce API call
        # burden at discovery time.
        ...
        while start < sync_end:
            ...
            if users.count > 1000:
                if search_window_size > 1:
                    ...
                    LOGGER.info(
                        "users - Detected Search API response size too large. Cutting search window"
                        " in half to %s seconds.",
                        search_window_size,
                    )
                    continue

                raise Exception((
                    "users - Unable to get all users within minimum window of a single second" +
                    " ({})\n" +
                    " found {} users within this timestamp. Zendesk can only provide a maximum" +
                    " of\n" +
                    " 1000 users per request. See:\n" +
                    " https://develop.zendesk.com/hc/en-us/articles/" +
                    "360022563994--BREAKING-New-Search-API-Result-Limits"
                ).format(parsed_start, users.count))
            ...
            if not all(parsed_start <= user.updated_at for user in users):
                ...
                if num_retries < 60:
                    LOGGER.info(
                        "users - Record found before date window start. Waiting 30 seconds, then"
                        " retrying window for consistency. (Retry #%s)",
                        num_retries + 1,
                    )
                    time.sleep(30)
                    num_retries += 1
                ...
                raise AssertionError(
                    "users - Record (user-id: {}) found before date window start and did not"
                    " resolve after 30 minutes of retrying. Details: window start ({}) is not less"
                    " than or equal to updated_at value(s) {}".format(
...
class Tickets(CursorBasedExportStream):
    ...
    def sync(self, state):  # pylint: disable=too-many-statements
        ...
        for ticket in tickets:
            ...
            # yielding stream name with record in a tuple as it is used for obtaining only the
            # parent records while sync
            ...
            if audits_stream.is_selected():
                try:
                    ...
                    # Skip stream if ticket_audit does not found for particular ticekt_id. Earlier
                    # it throwing HTTPError
            ...
            if metrics_stream.is_selected():
                try:
                    ...
                    # Skip stream if ticket_metric does not found for particular ticekt_id. Earlier
                    # it throwing HTTPError
                    # but now as error handling updated, it throws ZendeskNotFoundError.
                    message = (
            ...
            if comments_stream.is_selected():
                try:
                    ...
                    # Skip stream if ticket_comment does not found for particular ticekt_id. Earlier
                    # it throwing HTTPError
                    # but now as error handling updated, it throws ZendeskNotFoundError.
                    message = (
...
class TicketAudits(Stream):
    ...
    def check_access(self):
        ...
        try:
            ...
            # Skip 404 ZendeskNotFoundError error as goal is just to check whether TicketComments
            # have read permission or not
            pass
...
class TicketComments(Stream):
    ...
    def check_access(self):
        ...
        try:
            ...
            # Skip 404 ZendeskNotFoundError error as goal is to just check to whether TicketComments
            # have read permission or not
            pass
