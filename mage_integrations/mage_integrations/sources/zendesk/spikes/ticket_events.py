...
bookmark = datetime.strptime("2018-06-16T13:10:06+0000", "%Y-%m-%dT%H:%M:%S%z").astimezone(
    ZoneInfo("UTC")
)
...
# Both calls below appear to show that we could remove the nested code of Tickets and
# their Comments, Audits, and Metrics.

# Needs a newer version of Zenpy, but this might yield an issue singer-python.
# This spike used version 2.0.11

# Ticket Events Export appears to output Audits and if you sideload comment_events via
# include you also get comments.
for _ticket_event in client.tickets.events(start_time=1498775400, include=["comment_events"]):
    1 + 1

# Listing Ticket Metric Events retrieves metrics.
for _ticket_metric in client.tickets.metrics_incremental(start_time=1498775400):
    1 + 1
