...
def get_page_size(config, default_page_size):
    ...
    It will return the default value if an empty string is given, and
    will raise an exception if invalid value is given.
    """
...
def retry_pattern(fnc):
    @backoff.on_exception(backoff.constant, ConnectionError, max_tries=5, interval=60, jitter=None)
    @functools.wraps(fnc)
...
# Added decorator over functions of twitter SDK to perform backoff over SDK method Request.perform
# the method
...
class TwitterAds:
    ...
    def write_bookmark(self, state, stream, value, account_id, sub_type=None):
        ...
        state["bookmarks"][stream] = state["bookmarks"].get(
            stream, {}
        ...
        if sub_type:
            ...
            state["bookmarks"][stream][account_id] = (
                state["bookmarks"].get(stream, {}).get(account_id, {})
            )
            state["bookmarks"][stream][account_id][sub_type] = value
            LOGGER.info(
                "Stream: {} Subtype: {} - Write state, bookmark value: {}".format(
                    stream, sub_type, value
                )
            )
            ...
            state["bookmarks"][stream][
                account_id
            ] = value  # Update bookmark value for particular account
    ...
    def remove_hours_local(self, dttm, timezone):
        new_dttm = dttm.astimezone(timezone).replace(hour=0, minute=0, second=0, microsecond=0)
        return new_dttm

    def get_maximum_bookmark(
        self,
        bookmark_value_str,
        datetime_format,
        record_dict,
        ...
        """Return a maximum replication key value that is available in the
        record."""
    ...
    def sync_endpoint(
        ...
        id_fields = (
            hasattr(endpoint_config, "key_properties") or []
        ) and endpoint_config.key_properties
        ...
        bookmark_field = next(
            iter(
                (hasattr(endpoint_config, "replication_keys") or [])
                and endpoint_config.replication_keys
            ),
            None,
        )
        datetime_format = (
            hasattr(endpoint_config, "datetime_format") and endpoint_config.datetime_format
        )
        ...
        for sub_type in sub_types:
            if stream_name == "tweets" and last_datetime != start_date:
                # Tweets stream contains two separate bookmarks for each
                # sub_type(PUBLISHED, SCHEDULED)
            ...
            # cursor is an object like a generator(yield). First, it will be iterated for the
            # parent stream with the parent's bookmark. But, for the child also we want to
            # iterate through all parent records based on the child bookmark and collect
            # parent_ids. That's why we are making a cursor copy before the parent iteration.
            ...
            with metrics.record_counter(stream_name) as counter:
                # Sync only selected stream. When only child stream is selected(parent
                ...
                if stream_name in selected_streams:
                    # Loop thru cursor records, break out if no more data or
                    # bookmark_value < last_dttm
                    for record in cursor:
                        ...
                        if not record_dict:
                            # Finish looping
                            LOGGER.info(
                                "Stream: {} - Finished Looping, no more data".format(stream_name)
                            )
                            break
                        ...
                        if bookmark_field:
                            ...
                            if (
                                i == 0 and sub_type != "SCHEDULED"
                            ):  # SCHEDULED type tweets response do not contain records in sorted
                                # order. If first record then set it as max_bookmark_value
                                max_bookmark_value = max_bookmark_value_str

                            # Bookmark mechanism for SCHEDULED type tweets
                            if sub_type == "SCHEDULED":
                                # I have set max_bookmark_dttm = None, as it wasnt defined before.
                                # This is a temporary fix and should be analyzed further.
                                ...
                                if bookmark_value >= max_bookmark_dttm:
                                    # If replication key value of current record greater than
                                    # maximum bookmark then update it.
                                    max_bookmark_dttm = bookmark_value
                                    max_bookmark_value = max_bookmark_dttm.strftime(BOOKMARK_FORMAT)
                    ...
                    if stream_name == "tweets":
                        self.write_bookmark(
                            state, stream_name, max_bookmark_value, account_id, sub_type
                        )
                        max_bookmark_dttm = None
            ...
            if children:
                for child_stream_name in children:
                    ...
                    if child_stream_name in child_streams:
                        ...
                        child_last_datetime = self.get_bookmark(
                            state, child_stream_name, start_date, account_id
                        )
                        ...
                        for record in cursor_child:
                            ...
                            if bookmark_field:
                                ...
                                (
                                    ...
                                    max_bookmark_value_str,
                                ) = self.get_maximum_bookmark(
                                    bookmark_value_str,
                                    datetime_format,
                                    record_dict,
                                    bookmark_field,
                                ...
                                if child_bookmark_value < child_last_dttm:
                                    # Skip all records from now onwards because the replication key
                                    # value in record is less than last saved bookmark value.
                        ...
                        for chunk_ids in split_list(parent_ids, parent_id_limit):
                            # pylint: disable=line-too-long
                            LOGGER.info(
                                "Child Stream: {} - Syncing, chunk#: {}, parent_stream: {}, parent"
                                " chunk_ids: {}".format(
                        ...
                        # Update the state with the max_bookmark_value for
                        # the child stream if parent is incremental
                        if bookmark_field:
                            self.write_bookmark(
                                state, child_stream_name, child_max_bookmark_value, account_id
                            )
        ...
        # Update the state with the max_bookmark_value for all other
        # streams except tweets stream if stream is selected
...
class Reports(TwitterAds):
    ...
    def sync_report(
        ...
        while window_start != abs_end:
            ...
            async_results_urls = []
            async_results_urls = self.get_async_results_urls(
    ...
    def get_absolute_start_end_time(
        self, report_granularity, timezone, last_dttm, attribution_window
    ):
    ...
    def get_tweet_entity_ids(self, client, account_id, window_start, window_end):
        ...
        tweet_path = hasattr(tweet_config, "path") and tweet_config.path.replace(
            "{account_id}", account_id
        )
    ...
    #    country: country targeting value, required for segment = LOCATIONS, POSTAL_CODES, REGIONS,
    #      METROS?
    #    platform: platform targeting value, required for segment = DEVICES, PLATFORM_VERSIONS
    #  placements: required, 1 and only 1 per request; but loop thru 2: ALL_ON_TWITTER,
    #    PUBLISHER_NETWORK
    ...
    def post_queued_async_jobs(
        ...
        for chunk_ids in split_list(entity_ids, 20):
            ...
            LOGGER.info("Report: {} - POST ASYNC queued_job, chunk#: {}".format(report_name, chunk))
            queued_job_path = "stats/jobs/accounts/{account_id}".replace("{account_id}", account_id)
            queued_job_params = {
                # Required params
                "entity": report_entity,
            ...
            LOGGER.info("Report: {} - queued_job params: {}".format(report_name, queued_job_params))
    ...
    def get_async_results_urls(self, client, account_id, report_name, queued_job_ids):
        ...
        while len(queued_job_ids) > 0 and jobs_still_running and j <= 20:
            ...
            wait_sec = 15
            LOGGER.info(
                "Report: {} - Waiting {} sec for async job(s) to finish".format(
...
class Cards(TwitterAds):
    ...
    params = {
        ...
        "sort_by": ["updated_at-desc"],
        "with_deleted": "{with_deleted}",
        # As per the API document, the maximum page size is 1000 but API
        # throws an error if a page size passed more than 200
        "count": 200,
        "cursor": None,
    }
...
class TargetingLocations(TwitterAds):
    ...
    replication_method = "FULL_TABLE"
    sub_types = ["{country_code_list}"]
    params = {"country_code": "{sub_type}", "count": 1000, "cursor": None}
...
class TargetingTVShows(TwitterAds):
    ...
    replication_method = "FULL_TABLE"
    parent_ids_limit = 1
    params = {"locale": "{parent_ids}", "count": 50, "cursor": None}
