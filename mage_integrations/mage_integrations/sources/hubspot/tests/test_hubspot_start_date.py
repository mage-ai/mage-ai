...
class TestHubspotStartDate(HubspotBaseTest):
    ...
    def setUp(self):
        ...
        streams_under_test = self.expected_streams() - {
            'email_events'
        }  # we get this for free with subscription_changes
    ...
    def test_run(self):
        ...
        for stream in self.expected_streams():
            with self.subTest(stream=stream):
                ...
                first_sync_messages = first_sync_records.get(stream, {'messages': []}).get(
                    'messages'
                )
                second_sync_messages = second_sync_records.get(stream, {'messages': []}).get(
                    'messages'
                )
                first_sync_primary_keys = set(
                    tuple([record['data'][pk] for pk in primary_keys])
                    for record in first_sync_messages
                )
                second_sync_primary_keys = set(
                    tuple([record['data'][pk] for pk in primary_keys])
                ...
                if self.expected_metadata()[stream][self.OBEYS_START_DATE]:
                    ...
                    self.assertFalse(
                        first_sync_primary_keys.isdisjoint(second_sync_primary_keys),
                    ...
                    if replication_key and stream not in {
                        'contacts',
                        ...
                        if stream in {
                            ...
                            replication_key = (
                                [replication_key[0]]
                                if stream in ["deals", "companies"]
                                else [f'property_{replication_key[0]}']
                            )
                            first_sync_replication_key_values = [
                                record['data'][replication_key[0]]['value']
                                for record in first_sync_messages
                            ]
                            second_sync_replication_key_values = [
                                record['data'][replication_key[0]]['value']
                                for record in second_sync_messages
                            ]
                        else:
