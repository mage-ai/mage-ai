...
class TestClient:
    ...
    def _get_contacts_by_pks(self, pks):
        ...
        for vid, record in response_2.items():
            ...
            converted_ts = self.BaseTest.datetime_from_timestamp(ts_ms, self.BOOKMARK_DATE_FORMAT)
        ...
        for parent_id in parent_ids:
            ...
            while has_more:
                ...
                for vid in response.get('vids', {}):
                    records.extend([{'company-id': parent_id, 'contact-id': vid}])
    ...
    def get_deals(self):
        ...
        for batch in batches:
            data = {'inputs': batch, 'properties': v3_property}
        ...
        for v3_record in v3_records:
            for record in records:
                if v3_record['id'] == str(record['dealId']):
                    ...
                    non_null_v3_properties = {
                        v3_property_key: v3_property_value
                    ...
                    v3_properties = {
                        v3_property_key: {'value': v3_property_value}
    ...
    def get_email_events(self, recipient=''):
        ...
        while has_more:
            ...
            records.extend(
                [record for record in response['events'] if record['created'] >= self.start_date]
            )
    ...
    def get_forms(self):
        ...
        response = self.get(url)
        records.extend(
            [record for record in response if record[replication_key] >= self.start_date]
        )
    ...
    def get_workflows(self):
        ...
        records.extend(
            [
                record
                for record in response['workflows']
    ...
    def create_contacts(self):
        ...
        data = {
            "properties": [
                {"property": "email", "value": f"{record_uuid}@stitchdata.com"},
                {"property": "firstname", "value": "Yusaku"},
                {"property": "lastname", "value": "Kasahara"},
                {"property": "website", "value": "http://app.stitchdata.com"},
    ...
    def create_companies(self):
        ...
        data = {
            "properties": [
                {"name": "name", "value": f"Company Name {record_uuid}"},
    ...
    def create_engagements(self):
        ...
        contact_records = self.get_contacts()
        contact_ids = [
            contact['vid'] for contact in contact_records if contact['vid'] != 2304
        ]  # contact 2304 has hit the 10,000 assoc limit
    ...
    def create_subscription_changes(self, subscriptions=[], times=1):
        ...
        subscription_id_list = [
            [change.get('subscriptionId') for change in subscription['changes']]
    ...
    def create_workflows(self):
        ...
        data = {
            ...
            "enabled": True,
            "actions": [
                {"type": "DELAY", "delayMillis": 3600000},
                {"newValue": "HubSpot", "propertyName": "company", "type": "SET_CONTACT_PROPERTY"},
                {
                    "type": "WEBHOOK",
    ...
    def update_companies(self, company_id):
        ...
        data = {
            "properties": [
                {"name": "description", "value": f"An updated description {record_uuid}"}
            ]
        }
        self.put(url, data)
    ...
    def update_contacts(self, vid):
        ...
        data = {
            "properties": [
                {"property": "email", "value": f"{record_uuid}@stitchdata.com"},
                {"property": "firstname", "value": "Updated"},
                {"property": "lastname", "value": "Record"},
    ...
    def update_deal_pipelines(self, pipeline_id):
        ...
        record = [pipeline for pipeline in deal_pipelines if pipeline['pipelineId'] == pipeline_id][
            0
        ]
    ...
    def update_deals(self, deal_id):
        ...
        data = {
            "properties": [
                {"value": f"Updated {record_uuid}", "name": "dealname"},
            ]
        }
    ...
    def update_engagements(self, engagement_id):
        ...
        record_uuid = str(uuid.uuid4()).replace('-', '')[:20]
        data = {"metadata": {"body": f"Updated {record_uuid}"}}
    ...
    def delete_contact_lists(self, records=[], count=10):
        ...
        if len(record_ids_to_delete) == 1 or len(record_ids_to_delete) <= count:
            raise RuntimeError(
    ...
    def delete_deal_pipelines(self, records=[], count=10):
        ...
        for record_id in record_ids_to_delete:
            if (
                record_id == 'default' or len(record_id) > 16
            ):  # not a timestamp, not made by this client
                continue  # skip
    ...
    def __init__(self, start_date=''):
        ...
        stream_limitations = {
            'deal_pipelines': [100, deal_pipelines_records],
        ...
        for stream, limits in stream_limitations.items():
            ...
            if (
                max_record_count - pipeline_count
            ) / max_record_count <= 0.1:  # at/above 90% of record limit
