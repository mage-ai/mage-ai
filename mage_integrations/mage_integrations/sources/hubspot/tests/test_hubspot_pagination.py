...
class TestHubspotPagination(HubspotBaseTest):
    ...
    def get_properties(self):
        return {
            'start_date': datetime.strftime(
                datetime.today() - timedelta(days=7), self.START_DATE_FORMAT
            )
        }
    ...
    def setUp(self):
        ...
        stream_to_run_last = (
            'contacts_by_company'  # child stream depends on companyIds, must go last
        )
        ...
        for stream in streams:
            ...
            LOGGER.info(
                f"Pagination limit set to - {limits[stream]} and total number of existing record - {len(existing_records[stream])}"
            )
            ...
            if under_target > 0:
                ...
                if stream in {'subscription_changes', 'emails_events'}:
                    test_client.create(
                        stream, subscriptions=existing_records[stream], times=under_target
                    )
    ...
    def streams_to_test(self):
        ...
        streams_with_page_limits = {
            stream for stream, limit in self.expected_page_limits().items() if limit
        }
