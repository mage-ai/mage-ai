...
from mage_integrations.sources.hubspot.tap_hubspot.constants import (
    BOOKMARK_PROPERTIES_BY_STREAM_NAME,
)
...
def get_start(state, tap_stream_id, bookmark_key, older_bookmark_key=None):
    """If the current bookmark_key is available in the state, then return the
    bookmark_key value. If it is not available then check and return the
    older_bookmark_key in the state for the existing connection. If none of the
    keys are available in the state for a particular stream, then return
    start_date.

    We have made this change because of an update in the replication key
    of the deals stream. So, if any existing connections have only
    older_bookmark_key in the state then tap should utilize that
    bookmark value. Then next time, the tap should use the current
    bookmark value.
    """
...
def parse_custom_schema(entity_name, data):
    return {
        field["name"]: get_field_schema(field["type"], entity_name != "contacts") for field in data
    }
...
def get_params_and_headers(params):
    ...
    authentication values available.

    If there is an `hapikey` in the config, we
    ...
    if hapikey is None:
        if "access_token" not in CONFIG and (
            CONFIG["token_expires"] is None or CONFIG["token_expires"] < datetime.datetime.utcnow()
        ):
...
def post_search_endpoint(url, data, params=None):
    ...
    with metrics.http_request_timer(url) as _:
        resp = requests.post(
            url=url, json=data, params=params, timeout=get_request_timeout(), headers=headers
        )
...
def merge_responses(v1_data, v3_data):
    for v1_record in v1_data:
        ...
        for v3_record in v3_data:
            ...
            if str(v1_id) == v3_id:
                v1_record["properties"] = {**v1_record["properties"], **v3_record["properties"]}
...
def _sync_contact_vids(
    ...
    for record in data.values():
        ...
        singer.write_record(
            "contacts", record, catalog.get("stream_alias"), time_extracted=time_extracted
        )
...
default_company_params = {"limit": 250, "properties": ["createdate", "hs_lastmodifieddate"]}
...
def sync_companies(STATE, ctx, logger=LOGGER):
    ...
    start = utils.strptime_to_utc(
        get_start(STATE, "companies", bookmark_key, older_bookmark_key=bookmark_field_in_record)
    )
    ...
    with bumble_bee:
        for row in gen_request(
            ...
            if not modified_time or modified_time >= start:
                ...
                record = bumble_bee.transform(lift_properties_and_versions(record), schema, mdata)
                singer.write_record(
                    "companies", record, catalog.get("stream_alias"), time_extracted=utils.now()
                )
                if CONTACTS_BY_COMPANY in ctx.selected_stream_ids:
...
def has_selected_custom_field(mdata):
    ...
    for prop in top_level_custom_props:
        ...
        if (mdata.get(prop, {}).get("selected") is True) or (
            mdata.get(prop, {}).get("inclusion") == "automatic"
        ):
            return True
    return False
...
def sync_deals(STATE, ctx, logger=LOGGER):
    ...
    # As `hs_lastmodifieddate` is not available at the 1st level it can not be marked as automatic
    # inclusion.
    # tap includes all nested fields of the properties field as custom fields in the schema by
    # appending the prefix `property_` along with each field.
    # That's why bookmark_key is `property_hs_lastmodifieddate` so that we can mark it as automatic
    # inclusion.

    last_modified_date = "hs_lastmodifieddate"

    # Tap was used to write bookmark using replication key `hs_lastmodifieddate`.
    # Now, as the replication key gets changed to "property_hs_lastmodifieddate", `get_start`
    # function would return bookmark value of older bookmark key(`hs_lastmodifieddate`) if it is
    # available.
    # So, here `older_bookmark_key` is the previous bookmark key that may be available in the state
    # of the existing connection.

    start = utils.strptime_with_tz(
        get_start(STATE, "deals", bookmark_key, older_bookmark_key=last_modified_date)
    )
    max_bk_value = start
    ...
    with Transformer(UNIX_MILLISECONDS_INTEGER_DATETIME_PARSING) as bumble_bee:
        for row in gen_request(
            ...
            if not modified_time or modified_time >= start:
                ...
                singer.write_record(
                    "deals", record, catalog.get("stream_alias"), time_extracted=utils.now()
                )
...
# NB> no suitable bookmark is available:
# https://developers.hubspot.com/docs/methods/email/get_campaigns_by_id
def sync_campaigns(STATE, ctx, logger=LOGGER):
    ...
    with Transformer(UNIX_MILLISECONDS_INTEGER_DATETIME_PARSING) as bumble_bee:
        for row in gen_request(
            ...
            record = bumble_bee.transform(lift_properties_and_versions(record), schema, mdata)
            singer.write_record(
                "campaigns", record, catalog.get("stream_alias"), time_extracted=utils.now()
            )
    ...
    with metrics.record_counter(entity_name) as counter:
        while start_ts < now_ts:
            ...
            STATE = singer.write_bookmark(
                ...
                "startTimestamp",
                utils.strftime(
                    datetime.datetime.fromtimestamp((start_ts / 1000), datetime.timezone.utc)
                ),
            )  # pylint: disable=line-too-long
            singer.write_state(STATE)
            start_ts = end_ts
    ...
    with Transformer(UNIX_MILLISECONDS_INTEGER_DATETIME_PARSING) as bumble_bee:
        for row in gen_request(
            ...
            if record[bookmark_key] >= start:
                singer.write_record(
                    "contact_lists", record, catalog.get("stream_alias"), time_extracted=utils.now()
                )
...
def sync_forms(STATE, ctx, logger=LOGGER):
    ...
    with Transformer(UNIX_MILLISECONDS_INTEGER_DATETIME_PARSING) as bumble_bee:
        for row in data:
            ...
            if record[bookmark_key] >= start:
                singer.write_record(
                    "forms", record, catalog.get("stream_alias"), time_extracted=time_extracted
                )
            if record[bookmark_key] >= max_bk_value:
                max_bk_value = record[bookmark_key]
...
def sync_workflows(STATE, ctx, logger=LOGGER):
    ...
    with Transformer(UNIX_MILLISECONDS_INTEGER_DATETIME_PARSING) as bumble_bee:
        for row in data["workflows"]:
            ...
            if record[bookmark_key] >= start:
                singer.write_record(
                    "workflows", record, catalog.get("stream_alias"), time_extracted=time_extracted
                )
            if record[bookmark_key] >= max_bk_value:
                max_bk_value = record[bookmark_key]
...
def sync_owners(STATE, ctx, logger=LOGGER):
    ...
    with Transformer(UNIX_MILLISECONDS_INTEGER_DATETIME_PARSING) as bumble_bee:
        for row in data:
            ...
            if record[bookmark_key] >= start:
                singer.write_record(
                    "owners", record, catalog.get("stream_alias"), time_extracted=time_extracted
                )
...
def sync_deal_pipelines(STATE, ctx, logger=LOGGER):
    ...
    with Transformer(UNIX_MILLISECONDS_INTEGER_DATETIME_PARSING) as bumble_bee:
        for row in data:
            record = bumble_bee.transform(lift_properties_and_versions(row), schema, mdata)
            singer.write_record(
                "deal_pipelines", record, catalog.get("stream_alias"), time_extracted=utils.now()
            )
    singer.write_state(STATE)
    return STATE
...
STREAMS = [
    ...
    Stream(
        "companies", sync_companies, ["companyId"], "property_hs_lastmodifieddate", "INCREMENTAL"
    ),
...
def get_streams_to_sync(streams, state):
    ...
    result = streams
    if target_stream:
        skipped = list(itertools.takewhile(lambda x: x.tap_stream_id != target_stream, streams))
        rest = list(itertools.dropwhile(lambda x: x.tap_stream_id != target_stream, streams))
...
def load_discovered_schema(stream):
    ...
    # The engagements stream has nested data that we synthesize;
    # The engagement field needs to be automatic
    ...
    for stream in STREAMS:
        ...
        try:
            ...
            result["streams"].append(
                {
                    "bookmark_properties": BOOKMARK_PROPERTIES_BY_STREAM_NAME.get(
