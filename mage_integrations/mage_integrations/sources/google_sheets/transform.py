...
def transform_sheet_datetime_data(
    ...
    if isinstance(unformatted_value, (int, float)):
        # passing both the formatted as well as the unformatted value, so we can use the string
        # value in case of any errors while datetime transform
...
def transform_sheet_date_data(
    ...
    if isinstance(unformatted_value, (int, float)):
        # passing both the formatted as well as the unformatted value,
        # so we can use the string value in
...
def transform_sheet_time_data(
    ...
    if isinstance(unformatted_value, (int, float)):
        try:
            ...
            col_val = str(value)
            LOGGER.info(
                "WARNING: POSSIBLE DATA TYPE ERROR; SHEET: {}, COL: {}, CELL: {}{}, TYPE: {}"
                .format(sheet_title, col_name, col_letter, row_num, col_type)
            )
        return col_val
    else:
        return str(value)
...
def transform_sheet_boolean_data(
    ...
    if isinstance(value, bool):
        ...
        if value.lower() in ("true", "t", "yes", "y"):
            ...
            col_val = False
        # As the float and the int values would be now returned as
        # string itself, we need to check for the special
        # values as a string match rather than the integer/float match
            ...
            LOGGER.info(
                "WARNING: POSSIBLE DATA TYPE ERROR; SHEET: {}, COL: {}, CELL: {}{}, TYPE: {}"
                .format(sheet_title, col_name, col_letter, row, col_type)
            )
        return col_val
...
def transform_sheet_decimal_data(
    ...
    """Transform number type data and return according to the datatype in the
    sheet.
    ...
    try:
        ...
        # thus, we can convert "1.23E+03" to float but, for int casting we
        # get error and wrong value will be returned
        float(numeric_value)
    except ValueError:
        LOGGER.info(
            "Received the value for sheet: {}, column: {}, cell: {}{} with unexpected data type."
            " Ingesting this value with string format in the target".format(
...
def transform_sheet_number_data(
    ...
    """Handled number type records by type-casting into a float to verify the
    user has entered number type data.
