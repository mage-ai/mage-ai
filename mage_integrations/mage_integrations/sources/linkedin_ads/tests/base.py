...
class TestLinkedinAdsBase(unittest.TestCase):
    ...
    def setUp(self):
        missing_envs = [x for x in ["TAP_LINKEDIN_ADS_ACCOUNTS"] if os.getenv(x) is None]
        if missing_envs:
    ...
    def perform_and_verify_table_and_field_selection(
        self, conn_id, test_catalogs, select_all_fields=True, non_selected_properties=[]
    ):
        """
    ...
    def select_all_streams_and_fields(
        conn_id, catalogs, select_all_fields: bool = True, non_selected_properties=[]
    ):
