...
class TestAdsInsights(unittest.TestCase):
    fake_catalog_entry = CatalogEntry(
        schema={'properties': {'something': {'type': 'object'}}},
        metadata=[{'breadcrumb': ('properties', 'something'), 'metadata': {'selected': True}}],
    )
    ...
    def test_insights_start_dates_adjust_if_outside_window(self):
        ...
        expected_date = pendulum.today().subtract(
            months=AdsInsights.FACEBOOK_INSIGHTS_RETENTION_PERIOD
        )
        self.assertEqual(
            params[0]['time_ranges'],
...
class TestPrimaryKeyInclusion(unittest.TestCase):
    def test_primary_keys_automatically_included(self):
        streams = (
            tap_facebook.initialize_streams_for_discovery()
        )  # Make this list for the key_properties
        catalog = tap_facebook.discover_schemas()['streams']
        for catalog_entry in catalog:
            streamObject = [stream for stream in streams if stream.name == catalog_entry['stream']][
                0
            ]
            key_prop_breadcrumbs = {
                ('properties', x) for x in streamObject.key_properties
            }  # Enumerate the breadcrumbs for key properties
            for field in catalog_entry[
                'metadata'
            ]:  # Check that all key properties are automatic inclusion
...
def fake_args(is_discovery):
    ...
    def wrapped_function(*args, **kwargs):
        return fake_args(
            {'account_id': 123, 'access_token': 123}, is_discovery, {'streams': []}, {}
        )
