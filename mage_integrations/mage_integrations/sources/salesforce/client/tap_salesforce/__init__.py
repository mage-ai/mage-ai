...
parts = os.path.dirname(__file__).split('/')
sys.path.append('/'.join(parts[:-1]))
...
CONFIG = {'refresh_token': None, 'client_id': None, 'client_secret': None, 'start_date': None}
...
def discover_objects(sf, streams: List[str] = None) -> List[str]:
    ...
    return list(
        {o['name'] for o in global_description['sobjects'] if not streams or o['name'] in streams}
    )


# pylint: disable=too-many-branches,too-many-statements
def do_discover(sf, streams: List[str] = [], logger=LOGGER):
    ...
    for sobject_name in objects_to_discover:
        ...
        if missing_unsupported_field_names:
            logger.info(
                f'Ignoring the following unsupported fields for object {sobject_name} as they'
                f" are missing from the field list: {', '.join(sorted(missing_unsupported_field_names))}",
            )

        if filtered_unsupported_fields:
            filtered_unsupported_fields_str = ', '.join(
                sorted([k for k, _ in filtered_unsupported_fields])
            )
            logger.info(
                f'Not syncing the following unsupported fields for object {sobject_name}:'
        ...
        for prop, description in filtered_unsupported_fields:
            if metadata.get(mdata, ('properties', prop), 'selected'):
                metadata.delete(mdata, ('properties', prop), 'selected')
...
def do_sync(sf, catalog, state, sync_complete_callback=None, logger=LOGGER):
    ...
    for catalog_entry in catalog["streams"]:
        ...
        if job_id and batch_ids:
            with metrics.record_counter(stream) as counter:
                logger.info(
                    f'Found JobID from previous Bulk Query. Resuming sync for job: {job_id}'
                )
...
def main_impl():
    ...
    try:
        ...
        if args.discover:
            if query.get('list_objects'):
                json.dump(discover_objects(sf), sys.stdout, indent=2)
            else:
                do_discover(sf, streams=query.get('discover_streams'))
