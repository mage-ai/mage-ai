...
class SalesforceBookmarks(SalesforceBaseTest):
    ...
    def calculated_states_by_stream(self, current_state):
        ...
        stream_to_current_state = {
            stream: bookmark.get(self.expected_replication_keys()[stream].pop())
        ...
        for stream, state in stream_to_current_state.items():
            ...
            calculated_state = datetime.datetime.strftime(
                calculated_state_as_datetime, "%Y-%m-%dT%H:%M:%S.000000Z"
            )
    ...
    def test_run(self):
        ...
        streams_replication_methods = {stream: self.INCREMENTAL for stream in expected_streams}
        ...
        for stream in expected_streams:
            with self.subTest(stream=stream):
                ...
                self.assertEqual(
                    1,
                    len(list(replication_keys[stream])),
