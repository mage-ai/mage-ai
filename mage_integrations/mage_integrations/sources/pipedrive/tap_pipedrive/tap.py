...
class PipedriveTap(object):
        ...
        for stream in self.streams:
            ...
            if stream.id_list:  # see if we want to iterate over a list of deal_ids
                for deal_id in stream.get_deal_ids(self):
                    ...
                    if not is_last_id:
                        stream.more_items_in_collection = (
                            True  # set back to True for pagination of next deal_id request
                        )
                    elif (
            ...
            if stream.state_field:
                self.state = singer.write_bookmark(
                    self.state, stream.schema, stream.state_field, str(stream.earliest_state)
                )
            singer.write_state(self.state)
    ...
    @backoff.on_exception(
        backoff.expo, (Timeout, ConnectionError, PipedriveNull200Error), max_tries=5, factor=2
    )
    @backoff.on_exception(
        backoff.expo,
    ...
    @backoff.on_exception(
        ...
        (PipedriveTooManyRequestsInSecondError, PipedriveBadRequestError),
        giveup=is_not_status_code_fn([429]),
        jitter=None,
        max_tries=3,
    )
    def execute_request(self, endpoint, params=None):
        headers = {'User-Agent': self.config['user-agent'], 'Accept-Encoding': 'application/json'}
        _params = {
        ...
        try:
            ...
            if error_code == 429:
                ...
                if int(resp_headers.get("X-RateLimit-Remaining")) < 1:
                    message = (
                        "HTTP-error-code: 429, Error: {} Please retry after {} seconds.".format(
                ...
                message = (
                    "HTTP-error-code: 429, Error: Daily {} Please retry after {} seconds.".format(
                        api_rate_limit_message, resp_headers.get("X-RateLimit-Reset")
                    )
                )
