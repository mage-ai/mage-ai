...
class FreshdeskBookmarks(FreshdeskBaseTest):
    ...
    def calculated_states_by_stream(self, current_state):
        ...
        Sufficient test data is required for this test to cover a given
        stream. An incremental replication stream must have at least two
        records with replication keys that differ by some time span.

        If the test data is changed in the future this may break
        ...
        timedelta_by_stream = {
            stream: [0, 12, 0]  # {stream_name: [days, hours, minutes], ...}
        ...
        for stream, state_value in current_state["bookmarks"].items():
            if stream in bookmark_streams:
                ...
                calculated_state_formatted = datetime.strftime(
                    calculated_state_as_datetime, state_format
                )
                if calculated_state_formatted < self.start_date:
                    raise RuntimeError(
                        "Time delta error for stream {}, sim start_date < start_date!".format(
                            stream
                        )
                    )
    ...
    def test_run(self):
        """A Bookmarks Test."""
        # Since this tap has no table and field selection all streams will sync every time.
        # So define the subset of streams to run assertions against until all streams are covered
        self.test_streams = {"tickets", "companies", "agents", "groups", "roles", "conversations"}

        self.expected_replication_keys()
        ...
        if len(first_sync_empty) > 0:
            print(
                "Missing stream(s): {} in sync 1. Failing test for stream(s)".format(
                    first_sync_empty
                )
            )
        ...
        if len(second_sync_empty) > 0:
            print(
                "Missing stream(s): {} in sync 2. Failing test. Check test data!".format(
                    second_sync_empty
                )
            )
        ...
        for stream in self.test_streams:  # Add supported streams 1 by 1
            with self.subTest(stream=stream):
                ...
                if stream in self.first_sync_empty:
                    self.assertTrue(
                        False, msg="Stream: {} no longer in sync 1. Check test data".format(stream)
                    )
                ...
                if stream in self.second_sync_empty:
                    if stream == "conversations":
                        ...
                        # conversations is a child of tickets. When the child object
                        # (conversation / note) is updated, the parent object (ticket) is also
                        # updated.  The ticket is then being sync'd after update but the child
                        # conversation of that updated ticket is not (at least when the actual
                        # note text was updated.  There are several ways to update the child).
                        # self.assertTrue(False, msg="Stream: {} present in sync 1, missing in
                        # sync 2!".format(stream))

                        continue

                    self.assertTrue(
                        False, msg="Stream: {} present in sync 1, missing in sync 2!".format(stream)
                    )
                ...
                first_sync_records = [
                    record.get("data")
                    for record in first_sync_messages.get(stream).get("messages")
                    if record.get("action") == "upsert"
                ]
                second_sync_records = [
                    record.get("data")
                    for record in second_sync_messages.get(stream).get("messages")
                    if record.get("action") == "upsert"
                ]
                ...
                if expected_replication_method == self.INCREMENTAL:
                    ...
                    if stream == "conversations":
                        ...
                        # TODO discuss re-factor to use tickets bookmark for conversations
                        # assertions Verify the number of records in the 2nd sync is less
                        # then the first
                        self.assertLessEqual(second_sync_count, first_sync_count)
                        if second_sync_count == first_sync_count:
                            print(
                                "WARN: first_sync_count == second_sync_count for stream: {}".format(
                                    stream
                                )
                            )
                    ...
                    if stream == "roles":
                        ...
                        print(
                            "WARN: Less covereage, unable to update records for stream: {}".format(
                                stream
                            )
                        )
                    else:
                    ...
                    # Verify the bookmark is the max value sent to the target for a given
                    # replication key.
                    rec_time = []
                    ...
                    # Verify all replication key values in sync 2 are >= the simulated bookmark
                    # value.
                    for record in second_sync_records:
                        self.assertTrue(
                            record["updated_at"] >= simulated_states[stream],
