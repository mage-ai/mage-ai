...
class FreshdeskStartDateTest(FreshdeskBaseTest):
    ...
    def test_run(self):
        ...
        if len(first_sync_empty) > 0:
            print(
                "Missing stream: {} in sync 1. Failing test for stream(s). Add test data?".format(
                    first_sync_empty
                )
            )
        ...
        if len(second_sync_empty) > 0:
            print(
                "Missing stream(s): {} in sync 2. Updating expectations".format(second_sync_empty)
            )
        ...
        if len(second_sync_bonus) > 0:
            print(
                "Found stream(s): {} in second sync. Add to test_streams?".format(second_sync_bonus)
            )
            ...
            with self.subTest(stream=stream):
                ...
                if stream in self.second_sync_empty:
                    if stream == 'roles':
                        ...
                        print(
                            "No sync 2 data to compare for stream: {}, start_date obeyed".format(
                                stream
                            )
                        )
                ...
                expected_start_date_1 = self.timedelta_formatted(
                    self.start_date_1, days=0
                )  # Insight buffer format saved
