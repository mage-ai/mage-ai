...
class IncrementalStream(BaseStream):
    ...
    def sync(
        ...
        LOGGER.info(
            "Stream: {}, initial max_bookmark_value: {}".format(self.tap_stream_id, start_date)
        )
        ...
        with metrics.record_counter(self.tap_stream_id) as counter:
            for record in self.get_records(bookmark_datetime):
                ...
                record_datetime = singer.utils.strptime_to_utc(
                    self.epoch_milliseconds_to_dt_str(record[self.replication_key])
                )
                ...
                if record_datetime >= bookmark_datetime:
                    ...
                    singer.write_record(
                        self.tap_stream_id, transformed_record, time_extracted=singer.utils.now()
                    )
                    counter.increment()
                    max_datetime = max(record_datetime, max_datetime)
            bookmark_date = singer.utils.strftime(max_datetime)
            LOGGER.info(
                "FINISHED Syncing: {}, total_records: {}.".format(self.tap_stream_id, counter.value)
            )
...
class CompnaySegments(IncrementalStream):
    ...
    replication_key = 'updated_at'
    valid_replication_keys = ['updated_at']
    params = {'type': 'company', 'include_count': 'true'}
    data_key = 'segments'
...
class Conversations(IncrementalStream):
    ...
    def get_records(self, bookmark_datetime=None, is_parent=False) -> Iterator[list]:
        ...
        search_query = {
            ...
            "sort": {"field": self.replication_key, "order": "ascending"},
        }
        LOGGER.info("Syncing: {}".format(self.tap_stream_id))
        ...
        while paging:
            ...
            records = transform_json(response, self.tap_stream_id, self.data_key)
            LOGGER.info(
                "Synced: {} for page: {}, records: {}".format(
                    self.tap_stream_id,
        ...
        for record in parent.get_records(bookmark_datetime):
            ...
            LOGGER.info(
                "Syncing: {}, parent_stream: {}, parent_id: {}".format(
                    self.tap_stream_id, self.parent.tap_stream_id, record['id']
                )
            )
            ...
            LOGGER.info(
                "Synced: {}, parent_id: {}, records: {}".format(
            ...
            state = singer.write_bookmark(
                state, self.tap_stream_id, self.replication_key, parent_bookmark_value
            )
